...
public class KiemKhoFrame extends JFrame {
    ...
    private final DefaultTableModel stockModel = new DefaultTableModel(new Object[]{"Mã lô", "Nguyên liệu", "DVT", "Hệ thống", "HSD", "Trạng thái"}, 0) {
        public boolean isCellEditable(int r, int c) { return false; }
    };
    private final JTable stockTable = new JTable(stockModel);
    private final DefaultTableModel historyModel = new DefaultTableModel(new Object[]{"Mã phiếu", "Ngày", "Kho", "Trạng thái", "Dòng", "Người kiểm", "Ghi chú"}, 0) {
        public boolean isCellEditable(int r, int c) { return false; }
    };
    private final JTable historyTable = new JTable(historyModel);
    private final DefaultTableModel itemModel = new DefaultTableModel(new Object[]{"Lô", "Nguyên liệu", "DVT", "Hệ thống", "Thực tế", "Chênh lệch", "Xử lý", "Lý do"}, 0) {
        public boolean isCellEditable(int r, int c) { return false; }
    };
    ...
    private void buildDetailCard() {
        ...
        addLabel(card, "Hệ thống:", 260, 12, 120, 18); addField(card, systemQtyField, 260, 34, 130, 30, false);
        addLabel(card, "Thực tế:", 420, 12, 120, 18); addField(card, actualQtyField, 420, 34, 130, 30, true);
        addLabel(card, "Chênh lệch:", 580, 12, 120, 18); addField(card, diffQtyField, 580, 34, 130, 30, false);
        addLabel(card, "Hạn sử dụng:", 740, 12, 120, 18); addField(card, expiryField, 740, 34, 130, 30, false);
        addLabel(card, "Hướng xử lý:", 900, 12, 120, 18); addCombo(card, handlingCombo, 900, 34, 190, 30);
        addLabel(card, "Lý do:", 1120, 12, 100, 18); addField(card, reasonField, 1120, 34, 130, 30, true);
    ...
    private void bindEvents() {
        ...
        actualQtyField.getDocument().addDocumentListener(new javax.swing.event.DocumentListener() {
            ...
            public void removeUpdate(javax.swing.event.DocumentEvent e) { updateDiffField(); }
            public void changedUpdate(javax.swing.event.DocumentEvent e) { updateDiffField(); }
        });
    }
    ...
    private void loadLookups() {
        ...
        new SwingWorker<StocktakeLookupDto, Void>() {
            protected StocktakeLookupDto doInBackground() throws Exception { return apiClient.getLookups(); }
            protected void done() {
                try {
                    ...
                    ingredientCombo.setModel(new DefaultComboBoxModel<>(withAllOption("Tất cả nguyên liệu", lookup.getIngredients())));
                    statusCombo.setModel(new DefaultComboBoxModel<>(withAllOption("Tất cả trạng thái", lookup.getStatuses())));
                    handlingCombo.setModel(new DefaultComboBoxModel<>(lookup.getHandlingOptions().toArray(new OptionDto[0])));
                    ...
                    JOptionPane.showMessageDialog(KiemKhoFrame.this, unwrapMessage(ex), "Lỗi", JOptionPane.ERROR_MESSAGE);
                }
            }
        }.execute();
    }
        ...
        new SwingWorker<StocktakeScreenData, Void>() {
            ...
            protected void done() {
                try {
                    ...
                    lotCombo.setModel(new DefaultComboBoxModel<>(data.stocks().toArray(new StocktakeSystemStockDto[0])));
    ...
    private void populateStockTable(List<StocktakeSystemStockDto> rows) {
        ...
        for (StocktakeSystemStockDto row : rows) {
            stockModel.addRow(new Object[]{row.getMaLoHang(), valueOrDash(row.getTenNguyenLieu()), valueOrDash(row.getDonViTinh()), formatNumber(row.getSoLuongHeThong()), valueOrDash(row.getHanSuDung()), valueOrDash(row.getTrangThaiLo())});
        }
    }
    ...
    private void populateHistoryTable(List<StocktakeDto> rows) {
        ...
        for (StocktakeDto row : rows) {
            historyModel.addRow(new Object[]{row.getMaPhieuKiemKho(), formatDateTime(row.getNgayKiemKho()), valueOrDash(row.getTenKho()), statusLabelText(row.getTrangThai()), row.getSoDongChiTiet() == null ? 0 : row.getSoDongChiTiet(), valueOrDash(row.getTenNguoiKiem()), valueOrDash(row.getGhiChu())});
        }
    }
    ...
    private void populateItemTable() {
        ...
        for (StocktakeLine line : draftLines) {
            itemModel.addRow(new Object[]{line.maLoHang(), valueOrDash(line.tenNguyenLieu()), valueOrDash(line.donViTinh()), formatNumber(line.soLuongHeThong()), formatNumber(line.soLuongThucTe()), formatNumber(line.soLuongThucTe().subtract(line.soLuongHeThong())), handlingLabel(line.huongXuLy()), valueOrDash(line.lyDoChenhLech())});
        }
    }
    ...
    private void updateStockFieldsFromCombo() {
        ...
        if (stock == null) {
            systemQtyField.setText(""); diffQtyField.setText(""); expiryField.setText(""); return;
        }
        systemQtyField.setText(formatNumber(stock.getSoLuongHeThong()) + " " + valueOrDash(stock.getDonViTinh()));
    ...
    private void updateDiffField() {
        StocktakeSystemStockDto stock = (StocktakeSystemStockDto) lotCombo.getSelectedItem();
        if (stock == null || actualQtyField.getText().isBlank()) { diffQtyField.setText(""); return; }
        try {
            BigDecimal actual = parseDecimal(actualQtyField.getText());
            BigDecimal system = stock.getSoLuongHeThong() == null ? BigDecimal.ZERO : stock.getSoLuongHeThong();
    ...
    private void addOrUpdateLine() {
        ...
        if (stock == null) { showWarning("Vui lòng chọn lô cần kiểm kho"); return; }
        if (handling == null || handling.getCode() == null) { showWarning("Vui lòng chọn hướng xử lý"); return; }
        BigDecimal actual = parseNonNegativeDecimal(actualQtyField.getText(), "Số lượng thực tế phải lớn hơn hoặc bằng 0");
        if (actual == null) return;
        BigDecimal system = stock.getSoLuongHeThong() == null ? BigDecimal.ZERO : stock.getSoLuongHeThong();
        if ("CREATE_WASTAGE".equals(handling.getCode()) && actual.compareTo(system) >= 0) {
            showWarning("Tạo hao hụt chỉ áp dụng khi số lượng thực tế nhỏ hơn hệ thống"); return;
        }
        StocktakeLine line = new StocktakeLine(stock.getMaNguyenLieu(), stock.getTenNguyenLieu(), stock.getDonViTinh(), stock.getMaLoHang(), system, actual, reasonField.getText().trim(), handling.getCode());
        int existing = findDraftLineIndex(stock.getMaLoHang());
        if (existing >= 0) draftLines.set(existing, line); else draftLines.add(line);
        populateItemTable();
        actualQtyField.setText(""); reasonField.setText(""); updateStockFieldsFromCombo();
        statusLabel.setText("Đã thêm " + draftLines.size() + " dòng kiểm kho vào phiếu.");
    }

    private void removeSelectedLine() {
        int row = itemTable.getSelectedRow();
        if (row < 0 || row >= draftLines.size()) { showWarning("Vui lòng chọn dòng cần xóa"); return; }
    ...
    private void saveStocktake(boolean completeAfterCreate) {
        OptionDto warehouse = selectedOption(warehouseCombo);
        if (warehouse == null || warehouse.getId() == null) { showWarning("Vui lòng chọn kho kiểm"); return; }
        if (draftLines.isEmpty()) { showWarning("Phiếu kiểm kho cần ít nhất một dòng chi tiết"); return; }
        ...
        statusLabel.setText(completeAfterCreate ? "Đang lưu và hoàn tất phiếu kiểm kho..." : "Đang lưu phiếu kiểm kho...");
        new SwingWorker<StocktakeDto, Void>() {
            protected StocktakeDto doInBackground() throws Exception {
                StocktakeDto saved = editingStocktakeId == null
            ...
            protected void done() {
                ...
                try {
                    ...
                    JOptionPane.showMessageDialog(KiemKhoFrame.this, (completeAfterCreate ? "Đã hoàn tất phiếu kiểm kho #" : "Đã lưu phiếu kiểm kho #") + response.getMaPhieuKiemKho() + "\nKho: " + response.getTenKho() + "\nTrạng thái: " + statusLabelText(response.getTrangThai()), "Kiểm kho", JOptionPane.INFORMATION_MESSAGE);
                    resetDraft();
                    loadSystemStockAndHistory();
                } catch (Exception ex) {
    ...
    private void cancelSelectedStocktake() {
        ...
        int confirm = JOptionPane.showConfirmDialog(this, "Hủy phiếu kiểm kho #" + id + "?", "Kiểm kho", JOptionPane.YES_NO_OPTION);
        ...
        new SwingWorker<StocktakeDto, Void>() {
            ...
            protected void done() {
                ...
                try {
                    StocktakeDto response = get();
                    JOptionPane.showMessageDialog(KiemKhoFrame.this, "Đã hủy phiếu kiểm kho #" + response.getMaPhieuKiemKho(), "Kiểm kho", JOptionPane.INFORMATION_MESSAGE);
                    resetDraft();
                    loadSystemStockAndHistory();
                } catch (Exception ex) {
                    JOptionPane.showMessageDialog(KiemKhoFrame.this, unwrapMessage(ex), "Lỗi", JOptionPane.ERROR_MESSAGE);
    ...
    private void loadStocktakeIntoDraft(StocktakeDto stocktake) {
        ...
        if (!"DRAFT".equalsIgnoreCase(stocktake.getTrangThai())) {
            showWarning("Chỉ mở lại được phiếu nháp. Phiếu này đang ở trạng thái " + statusLabelText(stocktake.getTrangThai()));
            return;
        }
        ...
        if (stocktake.getItems() != null) {
            for (StocktakeApiClient.StocktakeItemDto item : stocktake.getItems()) {
                draftLines.add(new StocktakeLine(
    ...
    private List<StocktakeItemRequest> buildRequestItems() {
        ...
        for (StocktakeLine line : draftLines) {
            ...
            item.setMaNguyenLieu(line.maNguyenLieu()); item.setMaLoHang(line.maLoHang()); item.setSoLuongHeThong(line.soLuongHeThong()); item.setSoLuongThucTe(line.soLuongThucTe()); item.setLyDoChenhLech(line.lyDoChenhLech()); item.setHuongXuLy(line.huongXuLy());
            items.add(item);
        }
        return items;
    }
    ...
    private BigDecimal parseNonNegativeDecimal(String raw, String message) {
        if (raw == null || raw.isBlank()) { showWarning(message); return null; }
        try {
            BigDecimal value = parseDecimal(raw);
            if (value.compareTo(BigDecimal.ZERO) < 0) { showWarning(message); return null; }
            return value;
        } catch (NumberFormatException ex) { showWarning(message); return null; }
    }
    private BigDecimal parseDecimal(String raw) { return new BigDecimal(raw.trim().replace(",", "")); }
    ...
    private void configureTable(JTable table, int statusColumn) {
        table.setRowHeight(34); table.setFont(UiTheme.regular(13)); table.getTableHeader().setFont(UiTheme.bold(13)); table.getTableHeader().setBackground(TABLE_HEAD); table.getTableHeader().setForeground(TEXT); table.getTableHeader().setPreferredSize(new Dimension(0, 34)); table.setSelectionBackground(Color.decode("#F8DCC6")); table.setSelectionForeground(TEXT); table.setGridColor(BORDER); table.setShowGrid(true);
            ...
            public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int column) {
                ...
                if (!isSelected) {
                    if (statusColumn >= 0 && column == statusColumn) {
                        String text = String.valueOf(value); component.setForeground(text.contains("Hoàn") ? SUCCESS : text.contains("Hủy") ? DANGER : TEXT);
    ...
    private JScrollPane hiddenScrollPane(JTable table) {
        JScrollPane scrollPane = new JScrollPane(table); scrollPane.setBorder(BorderFactory.createLineBorder(SOFT_BORDER)); hideScrollBarsButKeepWheel(scrollPane); return scrollPane;
    }
    private void hideScrollBarsButKeepWheel(JScrollPane scrollPane) {
        scrollPane.setWheelScrollingEnabled(true); scrollPane.setVerticalScrollBarPolicy(ScrollPaneConstants.VERTICAL_SCROLLBAR_AS_NEEDED); scrollPane.setHorizontalScrollBarPolicy(ScrollPaneConstants.HORIZONTAL_SCROLLBAR_AS_NEEDED); scrollPane.getVerticalScrollBar().setPreferredSize(new Dimension(0, 0)); scrollPane.getHorizontalScrollBar().setPreferredSize(new Dimension(0, 0)); scrollPane.getVerticalScrollBar().setUnitIncrement(34);
    }
    ...
    private static RoundedButton secondaryButton(String text) { RoundedButton button = new RoundedButton(text).background(Color.decode("#B9B9B9")).hover(Color.decode("#A8A8A8")).radius(10); button.setForeground(TEXT); return button; }
    ...
    private String formatNumber(BigDecimal value) { return value == null ? "0" : numberFormat.format(value); }
    private String inputNumber(BigDecimal value) { return value == null ? "" : value.stripTrailingZeros().toPlainString(); }
    private String formatDateTime(String value) { if (value == null || value.isBlank()) return "-"; return value.length() > 19 ? value.substring(0, 19).replace('T', ' ') : value.replace('T', ' '); }
    private String valueOrDash(String value) { return value == null || value.isBlank() ? "-" : value; }
    private void showWarning(String message) { JOptionPane.showMessageDialog(this, message, "Kiểm kho", JOptionPane.WARNING_MESSAGE); }
    ...
    private record StocktakeScreenData(List<StocktakeSystemStockDto> stocks, List<StocktakeDto> history) {}
    private record StocktakeLine(Long maNguyenLieu, String tenNguyenLieu, String donViTinh, Long maLoHang, BigDecimal soLuongHeThong, BigDecimal soLuongThucTe, String lyDoChenhLech, String huongXuLy) {}

    private class LotComboRenderer extends DefaultListCellRenderer {
        public Component getListCellRendererComponent(JList<?> list, Object value, int index, boolean isSelected, boolean cellHasFocus) {
            JLabel label = (JLabel) super.getListCellRendererComponent(list, value, index, isSelected, cellHasFocus);
            if (value instanceof StocktakeSystemStockDto stock) label.setText("Lô #" + stock.getMaLoHang() + " - " + formatNumber(stock.getSoLuongHeThong()) + " " + valueOrDash(stock.getDonViTinh()));
            label.setFont(UiTheme.regular(14)); label.setBorder(BorderFactory.createEmptyBorder(0, 10, 0, 28)); label.setForeground(TEXT); label.setBackground(isSelected && index >= 0 ? Color.decode("#F8DCC6") : WHITE); return label;
        }
    }
}