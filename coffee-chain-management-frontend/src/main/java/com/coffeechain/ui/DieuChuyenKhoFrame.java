...
public class DieuChuyenKhoFrame extends JFrame {
  ...
  private final DefaultTableModel ingredientModel =
      new DefaultTableModel(new Object[] {"Mã NL", "Nguyên liệu", "DVT", "Tồn nguồn"}, 0) {
  ...
  private final DefaultTableModel transferModel =
      new DefaultTableModel(new Object[] {"Mã NL", "Nguyên liệu", "DVT", "Số lượng", "Lô"}, 0) {
        public boolean isCellEditable(int r, int c) {
          return false;
        }
      };
  private final JTable transferTable = new JTable(transferModel);
  private final DefaultTableModel lotModel =
      new DefaultTableModel(new Object[] {"Mã lô", "HSD", "Còn lại", "SL chuyển"}, 0) {
        public boolean isCellEditable(int r, int c) {
          return c == 3;
        }
      };
  ...
  private void buildHeader() {
    JLabel title = new JLabel("ĐIỀU CHUYỂN KHO");
  ...
  private void buildReceiptInfo() {
    ...
    RoundedPanel card = card(44, 104, 1352, 88);
    root.add(card);
    addLabel(card, "Kho nguồn:", 20, 12, 100, 18);
    addCombo(card, sourceCombo, 20, 34, 250, 34);
    sourceCombo.setRenderer(new WarehouseComboRenderer());
    addLabel(card, "Kho đích:", 310, 12, 100, 18);
    addCombo(card, destinationCombo, 310, 34, 250, 34);
    destinationCombo.setRenderer(new WarehouseComboRenderer());
    addLabel(card, "Ngày chuyển:", 600, 12, 120, 18);
    addField(card, dateField, 600, 34, 190, 34, false);
    addLabel(card, "Người tạo:", 830, 12, 100, 18);
    addField(card, creatorField, 830, 34, 190, 34, false);
    manualLotCheckBox.setBounds(1060, 38, 190, 24);
  ...
  private void buildTables() {
    root.add(sectionTitle("DANH SÁCH NGUYÊN LIỆU", 44, 216));
    configureTable(ingredientTable);
    JScrollPane left = hiddenScrollPane(ingredientTable);
    left.setBounds(44, 246, 560, 190);
    root.add(left);
    ...
    ingredientTable
        ...
        .addListSelectionListener(
            e -> {
              if (!e.getValueIsAdjusting()) selectIngredientFromTable();
            });
    root.add(sectionTitle("DÒNG ĐIỀU CHUYỂN", 650, 216));
    configureTable(transferTable);
    JScrollPane right = hiddenScrollPane(transferTable);
    right.setBounds(650, 246, 746, 190);
    root.add(right);
  }

  private void buildInputRow() {
    root.add(sectionTitle("CHI TIẾT ĐIỀU CHUYỂN", 44, 456));
    RoundedPanel card = card(44, 486, 1352, 92);
    root.add(card);
    addLabel(card, "Nguyên liệu:", 20, 12, 120, 18);
    addCombo(card, ingredientCombo, 20, 34, 360, 32);
    ingredientCombo.setRenderer(new IngredientRenderer());
    addLabel(card, "Số lượng:", 420, 12, 120, 18);
    addField(card, quantityField, 420, 34, 160, 32, true);
    RoundedButton addButton = primaryButton("Thêm vào phiếu");
    addButton.setBounds(620, 34, 150, 32);
    addButton.addActionListener(e -> addLine());
    card.add(addButton);
    RoundedButton removeButton = secondaryButton("Xóa dòng");
    removeButton.setBounds(790, 34, 100, 32);
    removeButton.addActionListener(e -> removeLine());
        ...
        .addTableModelListener(
            e -> {
              if (manualLotCheckBox.isSelected())
                quantityField.setText(formatInput(sumManualLots()));
            });
  }
  ...
  private void loadLookups() {
    ...
    statusLabel.setText("Đang tải dữ liệu điều chuyển...");
    new SwingWorker<InventoryTransferLookupDto, Void>() {
      protected InventoryTransferLookupDto doInBackground() throws Exception {
        return apiClient.getTransferLookups();
      }
      ...
      protected void done() {
        try {
          ...
          sourceCombo.setModel(
              new DefaultComboBoxModel<>(data.getSourceWarehouses().toArray(new OptionDto[0])));
          destinationCombo.setModel(
              new DefaultComboBoxModel<>(
                  data.getDestinationWarehouses().toArray(new OptionDto[0])));
          loading = false;
          setEnabledForm(true);
          loadStockForSource();
        } catch (Exception ex) {
  ...
  private void loadStockForSource() {
    OptionDto source = selectedOption(sourceCombo);
    if (source == null || source.getId() == null) return;
    loading = true;
    statusLabel.setText("Đang tải tồn kho nguồn...");
    new SwingWorker<List<InventoryStockDto>, Void>() {
      protected List<InventoryStockDto> doInBackground() throws Exception {
        return apiClient.getTransferStock(source.getId());
      }
      ...
      protected void done() {
        try {
          ...
          ingredientCombo.setModel(
              new DefaultComboBoxModel<>(sourceIngredients.toArray(new InventoryStockDto[0])));
          loading = false;
          loadLotsForSelectedIngredient();
          statusLabel.setText("Đã tải " + sourceIngredients.size() + " nguyên liệu còn tồn.");
        } catch (Exception ex) {
          loading = false;
          showError(ex);
        }
  ...
  private void loadLotsForSelectedIngredient() {
    ...
    OptionDto source = selectedOption(sourceCombo);
    InventoryStockDto ingredient = selectedIngredient();
    ...
    new SwingWorker<List<InventoryLotDto>, Void>() {
      protected List<InventoryLotDto> doInBackground() throws Exception {
        return apiClient.getTransferLots(source.getId(), ingredient.getId());
      }
  ...
  private void populateIngredientTable(List<InventoryStockDto> rows) {
    ...
    if (rows == null) return;
    for (InventoryStockDto row : rows)
      ingredientModel.addRow(
          new Object[] {
            row.getId(), row.getName(), row.getDescription(), format(row.getSoLuongTon())
          });
  }
  ...
  private void populateLotTable(List<InventoryLotDto> rows) {
    lotModel.setRowCount(0);
    if (rows == null) return;
    for (InventoryLotDto lot : rows)
      lotModel.addRow(
          new Object[] {
            lot.getMaLoHang(), valueOrDash(lot.getHanSuDung()), format(lot.getSoLuongConLai()), ""
          });
  }
  ...
  private void populateTransferTable() {
    ...
    for (TransferLine line : lines)
      transferModel.addRow(
          new Object[] {
            line.maNguyenLieu(),
            line.tenNguyenLieu(),
            line.donViTinh(),
            format(line.soLuong()),
            line.manualLots().isEmpty() ? "FEFO" : line.manualLots().size() + " lô"
          });
  }
  ...
  private void selectIngredientFromTable() {
    ...
    for (int i = 0; i < ingredientCombo.getItemCount(); i++) {
      InventoryStockDto item = ingredientCombo.getItemAt(i);
  ...
  private void addLine() {
    InventoryStockDto ingredient = selectedIngredient();
    if (ingredient == null) {
      warn("Vui lòng chọn nguyên liệu");
      return;
    }
    BigDecimal qty = parsePositive(quantityField.getText());
    if (qty == null) return;
    if (ingredient.getSoLuongTon() != null && qty.compareTo(ingredient.getSoLuongTon()) > 0) {
      warn("Số lượng điều chuyển không được vượt tồn kho nguồn");
      return;
    }
    List<ManualLot> manualLots =
        manualLotCheckBox.isSelected() ? collectManualLots() : new ArrayList<>();
    if (manualLotCheckBox.isSelected() && manualLots.isEmpty()) {
      warn("Vui lòng nhập số lượng theo lô hoặc tắt chọn lô thủ công");
      return;
    }
    if (manualLotCheckBox.isSelected() && sumManualLots().compareTo(qty) != 0) {
      warn("Tổng số lượng theo lô phải bằng số lượng điều chuyển");
      return;
    }
    TransferLine line =
        new TransferLine(
            ingredient.getId(), ingredient.getName(), ingredient.getDescription(), qty, manualLots);
    int idx = findLine(ingredient.getId());
    if (idx >= 0) lines.set(idx, line);
    else lines.add(line);
    populateTransferTable();
    quantityField.setText("");
    statusLabel.setText("Đã thêm " + lines.size() + " dòng điều chuyển.");
  }

  private void removeLine() {
    int row = transferTable.getSelectedRow();
    if (row < 0 || row >= lines.size()) {
      warn("Vui lòng chọn dòng cần xóa");
      return;
    }
    lines.remove(row);
    populateTransferTable();
  ...
  private void saveTransfer() {
    OptionDto source = selectedOption(sourceCombo);
    OptionDto dest = selectedOption(destinationCombo);
    if (source == null || dest == null) {
      warn("Vui lòng chọn kho nguồn và kho đích");
      return;
    }
    if (source.getId().equals(dest.getId())) {
      warn("Kho đích phải khác kho nguồn");
      return;
    }
    if (lines.isEmpty()) {
      warn("Phiếu điều chuyển cần ít nhất một dòng nguyên liệu");
      return;
    }
    CreateTransferReceiptRequest request = new CreateTransferReceiptRequest();
    request.setMaKhoNguon(source.getId());
    request.setMaKhoDich(dest.getId());
    request.setChonLoThuCong(manualLotCheckBox.isSelected());
    request.setGhiChu(noteArea.getText().trim());
    request.setItems(buildRequestItems());
    saveButton.setEnabled(false);
    statusLabel.setText("Đang lưu phiếu điều chuyển...");
    new SwingWorker<TransferReceiptDto, Void>() {
      protected TransferReceiptDto doInBackground() throws Exception {
        return apiClient.createTransferReceipt(request);
      }

      protected void done() {
        saveButton.setEnabled(true);
        try {
          TransferReceiptDto res = get();
          JOptionPane.showMessageDialog(
              DieuChuyenKhoFrame.this,
              "Đã tạo phiếu điều chuyển #"
                  + res.getMaPhieuDieuChuyen()
                  + "\n"
                  + res.getTenKhoNguon()
                  + " -> "
                  + res.getTenKhoDich(),
  ...
  private List<CreateTransferReceiptItemRequest> buildRequestItems() {
    ...
    for (TransferLine line : lines) {
      ...
      item.setSoLuongDieuChuyen(line.soLuong());
      List<TransferLotSelectionRequest> lots = new ArrayList<>();
      for (ManualLot lot : line.manualLots()) {
        TransferLotSelectionRequest req = new TransferLotSelectionRequest();
        req.setMaLoHang(lot.maLoHang());
        req.setSoLuongDieuChuyen(lot.soLuong());
        lots.add(req);
      }
      item.setLoHangDieuChuyen(lots);
      items.add(item);
    }
    return items;
  }
  ...
  private List<ManualLot> collectManualLots() {
    ...
    for (int i = 0; i < lotModel.getRowCount(); i++) {
      Object raw = lotModel.getValueAt(i, 3);
      if (raw == null || raw.toString().isBlank()) continue;
      BigDecimal qty = parsePositive(raw.toString());
      if (qty == null) return new ArrayList<>();
      Long lotId = Long.valueOf(lotModel.getValueAt(i, 0).toString());
      lots.add(new ManualLot(lotId, qty));
    }
    return lots;
  }

  private BigDecimal sumManualLots() {
    BigDecimal total = BigDecimal.ZERO;
    for (ManualLot lot : collectManualLots()) total = total.add(lot.soLuong());
    return total;
  }

  private int findLine(Long ingredientId) {
    for (int i = 0; i < lines.size(); i++)
      if (ingredientId.equals(lines.get(i).maNguyenLieu())) return i;
    return -1;
  }

  private InventoryStockDto selectedIngredient() {
    return (InventoryStockDto) ingredientCombo.getSelectedItem();
  }
  ...
  private BigDecimal parsePositive(String raw) {
    try {
      if (raw == null || raw.isBlank()) {
        warn("Số lượng phải lớn hơn 0");
        return null;
      }
  ...
  private void warn(String message) {
    JOptionPane.showMessageDialog(this, message, "Điều chuyển kho", JOptionPane.WARNING_MESSAGE);
  }

  private void showError(Exception ex) {
    JOptionPane.showMessageDialog(this, unwrapMessage(ex), "Lỗi", JOptionPane.ERROR_MESSAGE);
    statusLabel.setText("Không xử lý được yêu cầu");
  }
  ...
  private RoundedPanel card(int x, int y, int w, int h) {
    RoundedPanel p = new RoundedPanel(12, WHITE, SOFT_BORDER);
    p.setLayout(null);
    p.setBounds(x, y, w, h);
    return p;
  }
  ...
  private class WarehouseComboRenderer extends DefaultListCellRenderer {
    ...
    public Component getListCellRendererComponent(
      ...
      JLabel label =
          (JLabel) super.getListCellRendererComponent(list, value, index, selected, focus);
  ...
  private class IngredientRenderer extends DefaultListCellRenderer {
    public Component getListCellRendererComponent(
        JList<?> list, Object value, int index, boolean selected, boolean focus) {
      JLabel l = (JLabel) super.getListCellRendererComponent(list, value, index, selected, focus);
      if (value instanceof InventoryStockDto i)
        l.setText(
            i.getName()
                + " - còn "
                + format(i.getSoLuongTon())
                + " "
                + valueOrDash(i.getDescription()));
      return l;
    }
  }
