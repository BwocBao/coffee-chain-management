...
public class TheoDoiHanSuDungFrame extends JFrame {
    ...
    private final DefaultTableModel lotTableModel = new DefaultTableModel(
            new Object[]{"Mã lô", "Kho", "Nguyên liệu", "DVT", "Còn lại", "Hạn sử dụng", "Còn ngày", "Cảnh báo"}, 0
    ) {
        @Override
    ...
    private void buildHeader() {
        ...
        JLabel subtitle = new JLabel("Theo dõi lô nguyên liệu theo FEFO, cảnh báo sắp hết hạn và rà soát lô quá hạn");
    ...
    private void loadData(boolean loadLookups) {
        ...
        new SwingWorker<ExpiryScreenData, Void>() {
            ...
            protected ExpiryScreenData doInBackground() throws Exception {
                ...
                List<ExpiryLotDto> lots = apiClient.searchLots(
                        maKho,
                        maNguyenLieu,
                        trangThai,
                        mucCanhBao,
    ...
    private List<ExpiryLotDto> filterByKeyword(List<ExpiryLotDto> rows) {
        ...
        for (ExpiryLotDto item : rows) {
            String haystack = (String.valueOf(item.getMaLoHang()) + " "
                    + valueOrDash(item.getTenKho()) + " "
                    + valueOrDash(item.getTenNguyenLieu()) + " "
                    + lotStatusLabel(item.getTrangThai()) + " "
                    + warningLabel(item.getMucCanhBao())).toLowerCase(Locale.ROOT);
    ...
    private Integer parseInteger(String raw, String label) {
        try {
            ...
            JOptionPane.showMessageDialog(this, label + " phải là số nguyên không âm", "Dữ liệu chưa hợp lệ", JOptionPane.WARNING_MESSAGE);
            return null;
        }
    }
    ...
    private record ExpiryScreenData(ExpiryLookupDto lookups, List<ExpiryLotDto> lots, ExpiryStatisticsDto statistics) {}
