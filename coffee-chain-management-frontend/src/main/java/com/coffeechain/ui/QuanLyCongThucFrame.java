...
public class QuanLyCongThucFrame extends JFrame {
    ...
    private final JTextField searchField = new IconPlaceholderTextField("Tìm công thức", new FlatSVGIcon("icons/tim.svg", 18, 18));
    ...
    private final JLabel selectedMeta = new JLabel("Chọn một dòng bên trái hoặc bấm Thêm mới công thức");
    ...
    private final DefaultTableModel summaryModel = new DefaultTableModel(new Object[]{"Mã CT", "Sản phẩm", "Trạng thái", "Ngày tạo", "SL NL"}, 0) {
        @Override public boolean isCellEditable(int row, int column) { return false; }
    };
    private final JTable summaryTable = new JTable(summaryModel);
    private final DefaultTableModel ingredientModel = new DefaultTableModel(new Object[]{"STT", "Nguyên liệu", "ĐVT", "Định mức", "Giá vốn", "Thành tiền"}, 0) {
        @Override public boolean isCellEditable(int row, int column) { return false; }
    };
    ...
    private void buildHeader() {
        ...
        backButton.addActionListener(e -> { new QuanLyPOSFrame().setVisible(true); dispose(); });
        root.add(backButton);
    }
...
//        addCombo(card, statusCombo, 394, 138, 150, 34);
        ...
        imagePreview.addMouseListener(new MouseAdapter() {
            @Override public void mouseClicked(MouseEvent e) { chooseAndUploadImage(); }
        });
        ...
        card.add(saveButton); card.add(clearButton); card.add(stopSellingButton); card.add(deleteFormulaButton);
    }
    ...
    private void bindEvents() {
        ...
        filterStatusCombo.addActionListener(e -> { if (!loading) loadRecipes(); });
        summaryTable.getSelectionModel().addListSelectionListener(e -> { if (!e.getValueIsAdjusting()) loadSelectedDetail(); });
    }
    ...
    private void loadLookupsAndData() {
        ...
        new SwingWorker<RecipeLookupDto, Void>() {
            @Override protected RecipeLookupDto doInBackground() throws Exception { return apiClient.getLookups(); }
            @Override protected void done() {
                try {
                    RecipeLookupDto lookup = get();
                    ingredients.clear(); statuses.clear();
                    if (lookup != null && lookup.getIngredients() != null) ingredients.addAll(lookup.getIngredients());
                    if (lookup != null && lookup.getStatuses() != null) statuses.addAll(lookup.getStatuses());
                    populateCombos();
                    loadRecipes();
                } catch (Exception ex) { showError("Không tải được lookup", ex); }
                finally { loading = false; applyPermissions(); }
            }
        }.execute();
    }
    ...
    private void populateCombos() {
        ...
        for (StatusOption status : statuses) { filterStatusCombo.addItem(status); statusCombo.addItem(status); }
    ...
    private void loadRecipes() {
        ...
        new SwingWorker<List<RecipeSummaryDto>, Void>() {
            @Override protected List<RecipeSummaryDto> doInBackground() throws Exception { return apiClient.searchRecipes(searchText(), selectedFilterStatus()); }
            @Override protected void done() {
                try {
                    recipes.clear(); recipes.addAll(get());
                    populateSummaryTable();
                    showStatus("Tổng số: " + recipes.size() + " công thức", false);
                } catch (Exception ex) { showError("Tra cứu công thức thất bại", ex); }
                finally { loading = false; applyPermissions(); }
            }
        }.execute();
    }
    ...
    private void populateSummaryTable() {
        ...
        for (RecipeSummaryDto r : recipes) summaryModel.addRow(new Object[]{valueOrDash(r.getMaCongThucHienThi()), valueOrDash(r.getTenSanPham()), statusText(r.getTrangThai()), formatDate(r.getNgayTao()), r.getSoNguyenLieu() == null ? 0 : r.getSoNguyenLieu()});
    }
    ...
    private void loadSelectedDetail() {
        ...
        new SwingWorker<RecipeDetailDto, Void>() {
            @Override protected RecipeDetailDto doInBackground() throws Exception { return apiClient.getRecipeDetail(summary.getMaSanPham()); }
            @Override protected void done() { try { fillDetail(get()); } catch (Exception ex) { showError("Không tải được chi tiết công thức", ex); } }
        }.execute();
    }
    ...
    private void fillDetail(RecipeDetailDto detail) {
        ...
        selectedMeta.setText(valueOrDash(detail.getMaCongThucHienThi()) + "  •  " + statusText(detail.getTrangThai()) + "  •  " + formatDate(detail.getNgayTao()));
    ...
    private void clearForm() {
        ...
        productNameField.setText(""); imageUrl = null; imagePreview.clear(); priceField.setText(""); qtyField.setText("");
        if (statusCombo.getItemCount() > 0) statusCombo.setSelectedIndex(0);
        currentLines.clear(); refreshIngredientTable(); updatePricing(); applyPermissions();
    }

    private void addIngredientLine() {
        IngredientOption ingredient = (IngredientOption) ingredientCombo.getSelectedItem();
        if (ingredient == null || ingredient.getId() == null) { showWarning("Vui lòng chọn nguyên liệu"); return; }
        BigDecimal qty = parsePositive(qtyField.getText(), "Định mức");
        if (qty == null) return;
        for (RecipeIngredientLineDto line : currentLines) if (ingredient.getId().equals(line.getMaNguyenLieu())) { showWarning("Nguyên liệu đã có trong công thức"); return; }
        RecipeIngredientLineDto line = new RecipeIngredientLineDto();
        line.setMaNguyenLieu(ingredient.getId()); line.setTenNguyenLieu(ingredient.getName()); line.setDonViTinh(ingredient.getUnit()); line.setSoLuongCan(qty); line.setGiaVonDvt(ingredient.getGiaVonDvt()); line.setThanhTien(qty.multiply(nullToZero(ingredient.getGiaVonDvt())));
        currentLines.add(line); qtyField.setText(""); refreshIngredientTable(); updatePricing();
    }

    private void removeIngredientLine() {
        int row = ingredientTable.getSelectedRow();
        if (row < 0) { showWarning("Vui lòng chọn dòng nguyên liệu cần xóa"); return; }
        currentLines.remove(ingredientTable.convertRowIndexToModel(row)); refreshIngredientTable(); updatePricing();
    }
    ...
    private void refreshIngredientTable() {
        ...
        for (RecipeIngredientLineDto line : currentLines) {
            BigDecimal thanhTien = nullToZero(line.getSoLuongCan()).multiply(nullToZero(line.getGiaVonDvt()));
            line.setThanhTien(thanhTien);
            ingredientModel.addRow(new Object[]{i++, valueOrDash(line.getTenNguyenLieu()), valueOrDash(line.getDonViTinh()), formatQty(line.getSoLuongCan()), formatMoney(line.getGiaVonDvt()), formatMoney(thanhTien)});
        }
    }

    private void updatePricing() {
        BigDecimal total = BigDecimal.ZERO;
        for (RecipeIngredientLineDto line : currentLines) total = total.add(nullToZero(line.getThanhTien()));
        ...
        if (sale.compareTo(BigDecimal.ZERO) > 0) grossMarginLabel.setText(sale.subtract(total).multiply(BigDecimal.valueOf(100)).divide(sale, 1, RoundingMode.HALF_UP) + "%");
    ...
    private void saveRecipe() {
        ...
        setButtonsEnabled(false);
        new SwingWorker<RecipeDetailDto, Void>() {
            @Override protected RecipeDetailDto doInBackground() throws Exception { return creating ? apiClient.createRecipe(req) : apiClient.updateRecipe(id, req); }
            @Override protected void done() { try { RecipeDetailDto d = get(); fillDetail(d); loadRecipes(); showStatus("Đã lưu công thức", false); } catch (Exception ex) { showError("Lưu công thức thất bại", ex); } finally { applyPermissions(); } }
        }.execute();
    }

    private RecipeRequest buildRequest() {
        if (productNameField.getText().trim().isEmpty()) { showWarning("Vui lòng nhập tên sản phẩm"); return null; }
        BigDecimal price = parsePositive(priceField.getText(), "Giá bán"); if (price == null) return null;
        if (currentLines.isEmpty()) { showWarning("Công thức phải có ít nhất một nguyên liệu"); return null; }
        RecipeRequest req = new RecipeRequest();
        req.setTenSanPham(productNameField.getText().trim()); req.setHinhAnh(imageUrl == null ? null : imageUrl.trim()); req.setGiaBanHienTai(price); req.setTrangThai(selectedStatus());
        List<RecipeIngredientRequest> items = new ArrayList<>();
        for (RecipeIngredientLineDto line : currentLines) items.add(new RecipeIngredientRequest(line.getMaNguyenLieu(), line.getSoLuongCan()));
        req.setItems(items); return req;
    }

    private void deleteFormula() {
        if (selectedProductId == null) { showWarning("Vui lòng chọn công thức"); return; }
        if (JOptionPane.showConfirmDialog(this, "Xóa công thức hiện hành của sản phẩm này?", "Xóa công thức", JOptionPane.YES_NO_OPTION) != JOptionPane.YES_OPTION) return;
        Long id = selectedProductId;
        new SwingWorker<RecipeDetailDto, Void>() { @Override protected RecipeDetailDto doInBackground() throws Exception { return apiClient.deleteFormula(id); } @Override protected void done() { try { fillDetail(get()); loadRecipes(); } catch (Exception ex) { showError("Xóa công thức thất bại", ex); } } }.execute();
    }

    private void stopSelling() {
        if (selectedProductId == null) { showWarning("Vui lòng chọn sản phẩm"); return; }
        Long id = selectedProductId;
        new SwingWorker<RecipeDetailDto, Void>() { @Override protected RecipeDetailDto doInBackground() throws Exception { return apiClient.stopSelling(id); } @Override protected void done() { try { fillDetail(get()); loadRecipes(); } catch (Exception ex) { showError("Ngừng bán thất bại", ex); } } }.execute();
    }
    ...
    private void chooseAndUploadImage() {
        ...
        chooser.setFileFilter(new FileNameExtensionFilter("Hình ảnh", "jpg", "jpeg", "png", "webp", "gif"));
        ...
        new SwingWorker<UploadImageResponse, Void>() {
            @Override protected UploadImageResponse doInBackground() throws Exception {
    ...
    private String searchText() { return searchField.getText().trim(); }
    private String selectedFilterStatus() { StatusOption s = (StatusOption) filterStatusCombo.getSelectedItem(); return s == null ? null : s.getCode(); }
    private String selectedStatus() { StatusOption s = (StatusOption) statusCombo.getSelectedItem(); return s == null ? "AVAILABLE" : s.getCode(); }
    private void selectStatus(String code) { for (int i = 0; i < statusCombo.getItemCount(); i++) if (code != null && code.equals(statusCombo.getItemAt(i).getCode())) { statusCombo.setSelectedIndex(i); return; } }
    private void applyPermissions() { boolean canManage = PermissionUtil.hasAny("RECIPE:MANAGE"); setButtonsEnabled(canManage && !loading); }
    private void setButtonsEnabled(boolean enabled) { saveButton.setEnabled(enabled); deleteFormulaButton.setEnabled(enabled && selectedProductId != null); stopSellingButton.setEnabled(enabled && selectedProductId != null); clearButton.setEnabled(enabled); addIngredientButton.setEnabled(enabled); removeLineButton.setEnabled(enabled); newButton.setEnabled(!loading && PermissionUtil.hasAny("RECIPE:MANAGE")); }

    private BigDecimal parsePositive(String raw, String label) { try { BigDecimal v = new BigDecimal(raw.trim().replace(",", "")); if (v.compareTo(BigDecimal.ZERO) <= 0) throw new NumberFormatException(); return v; } catch (Exception e) { showWarning(label + " phải là số lớn hơn 0"); return null; } }
    private BigDecimal parseNumberOrZero(String raw) { try { return new BigDecimal(raw.trim().replace(",", "")); } catch (Exception e) { return BigDecimal.ZERO; } }
    private BigDecimal nullToZero(BigDecimal v) { return v == null ? BigDecimal.ZERO : v; }
    private String inputNumber(BigDecimal v) { return v == null ? "" : v.stripTrailingZeros().toPlainString(); }
    private String formatMoney(BigDecimal v) { return moneyFormat.format(nullToZero(v)); }
    private String formatQty(BigDecimal v) { return qtyFormat.format(nullToZero(v)); }
    private String formatDate(java.time.LocalDateTime dt) { return dt == null ? "-" : dateFormat.format(dt); }
    private String statusText(String code) { if ("AVAILABLE".equals(code)) return "Có sẵn"; if ("OUT_OF_STOCK".equals(code)) return "Chưa có công thức"; if ("STOP_SELLING".equals(code)) return "Ngừng bán"; return valueOrDash(code); }
    ...
    private void showStatus(String msg, boolean error) { statusLabel.setForeground(error ? DANGER : MUTED); statusLabel.setText(msg); }
    private void showWarning(String msg) { JOptionPane.showMessageDialog(this, msg, "Quản lý công thức", JOptionPane.WARNING_MESSAGE); }
    private void showError(String title, Exception ex) { String msg = unwrapMessage(ex); showStatus(title + ": " + msg, true); JOptionPane.showMessageDialog(this, msg, "Lỗi", JOptionPane.ERROR_MESSAGE); }
    ...
    private void addFieldPanel(JPanel parent, JTextField field, int x, int y, int w, int h) { styleField(field); RoundedInputPanel p = new RoundedInputPanel(); p.setLayout(new BorderLayout()); p.setBounds(x, y, w, h); p.add(field, BorderLayout.CENTER); parent.add(p); }
    private void addCombo(JPanel parent, JComboBox<?> combo, int x, int y, int w, int h) { styleCombo(combo); OutlinedInputPanel p = new OutlinedInputPanel(); p.setLayout(new BorderLayout()); p.setBounds(x, y, w, h); p.add(combo, BorderLayout.CENTER); parent.add(p); }
    private void addLabel(JPanel p, String text, int x, int y, int w, int h) { JLabel l = new JLabel(text); l.setBounds(x, y, w, h); l.setForeground(MUTED); l.setFont(UiTheme.regular(13)); p.add(l); }
    private JLabel sectionTitle(String text) { JLabel l = new JLabel(text); l.setForeground(TEXT); l.setFont(UiTheme.bold(16)); return l; }
    private void styleField(JTextField f) { f.setFont(UiTheme.regular(14)); f.setForeground(TEXT); f.setBorder(BorderFactory.createEmptyBorder(0, f instanceof IconPlaceholderTextField ? 48 : 12, 0, 12)); f.setOpaque(false); }
    private void styleCombo(JComboBox<?> c) { c.setUI(new DesignComboBoxUI()); c.setRenderer(new DesignComboBoxRenderer()); c.setFont(UiTheme.regular(14)); c.setBackground(WHITE); c.setForeground(TEXT); c.setBorder(BorderFactory.createEmptyBorder(0,12,0,8)); c.setOpaque(false); c.setFocusable(false); c.setMaximumRowCount(8); }
    private void configureTable(JTable t) { t.setRowHeight(40); t.setFont(UiTheme.regular(13)); t.getTableHeader().setFont(UiTheme.bold(13)); t.getTableHeader().setBackground(TABLE_HEAD); t.getTableHeader().setPreferredSize(new Dimension(0,38)); t.setSelectionBackground(Color.decode("#F8DCC6")); t.setGridColor(SOFT_BORDER); t.setShowGrid(true); t.setDefaultRenderer(Object.class, new DefaultTableCellRenderer(){ @Override public Component getTableCellRendererComponent(JTable table,Object value,boolean isSelected,boolean hasFocus,int row,int col){ Component c=super.getTableCellRendererComponent(table,value,isSelected,hasFocus,row,col); setBorder(BorderFactory.createEmptyBorder(0,10,0,10)); if(!isSelected){ c.setForeground(TEXT); if(col==2 && table==summaryTable){ String v=String.valueOf(value); c.setForeground(v.contains("Có")?SUCCESS:v.contains("Chưa")?WARNING:v.contains("Ngừng")?DANGER:TEXT); } } return c; }}); }
    private JScrollPane hiddenScrollPane(JTable t) { JScrollPane sp = new JScrollPane(t); sp.setBorder(BorderFactory.createLineBorder(SOFT_BORDER)); sp.setWheelScrollingEnabled(true); sp.setVerticalScrollBarPolicy(ScrollPaneConstants.VERTICAL_SCROLLBAR_AS_NEEDED); sp.setHorizontalScrollBarPolicy(ScrollPaneConstants.HORIZONTAL_SCROLLBAR_AS_NEEDED); sp.getVerticalScrollBar().setPreferredSize(new Dimension(0,0)); sp.getHorizontalScrollBar().setPreferredSize(new Dimension(0,0)); sp.getVerticalScrollBar().setUnitIncrement(34); return sp; }
    ...
    private static RoundedButton secondaryButton(String text) { RoundedButton b = new RoundedButton(text).background(Color.decode("#D8D8D8")).hover(Color.decode("#C5C5C5")).radius(10); b.setForeground(TEXT); return b; }
    ...
    private static RoundedButton outlineButton(String text) { RoundedButton b = new RoundedButton(text).background(WHITE).hover(Color.decode("#FFF4EA")).radius(10); b.setForeground(PRIMARY_DARK); b.setBorder(BorderFactory.createLineBorder(PRIMARY)); return b; }
    ...
    private static class ImagePreviewPanel extends JPanel {
        ...
        @Override protected void paintComponent(Graphics g) {
            ...
            if (image != null) {
                double scale = Math.min((getWidth() - 16) / (double) image.getWidth(), (getHeight() - 16) / (double) image.getHeight());
                ...
                g2.setClip(new java.awt.geom.RoundRectangle2D.Float(4, 4, getWidth() - 8, getHeight() - 8, 8, 8));
    ...
    private static class DesignComboBoxRenderer extends DefaultListCellRenderer {
        @Override public Component getListCellRendererComponent(JList<?> list, Object value, int index, boolean isSelected, boolean cellHasFocus) {
            ...
            if (value instanceof StatusOption status) label.setText(status.getName() == null ? status.getCode() : status.getName());
    ...
    private static class OutlinedInputPanel extends JPanel {
        OutlinedInputPanel() { setOpaque(false); }
        @Override protected void paintComponent(Graphics g) {
            Graphics2D g2 = (Graphics2D) g.create();
            g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
            g2.setColor(WHITE);
    ...
    private static class RoundedCard extends JPanel { private final int r; private final Color fill; private final Color border; RoundedCard(int r, Color fill, Color border){this.r=r;this.fill=fill;this.border=border;setOpaque(false);} @Override protected void paintComponent(Graphics g){ Graphics2D g2=(Graphics2D)g.create(); g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING,RenderingHints.VALUE_ANTIALIAS_ON); g2.setColor(fill); g2.fillRoundRect(0,0,getWidth()-1,getHeight()-1,r,r); g2.setColor(border); g2.drawRoundRect(0,0,getWidth()-1,getHeight()-1,r,r); g2.dispose(); super.paintComponent(g);} }
    private static class RoundedInputPanel extends JPanel { RoundedInputPanel(){setOpaque(false);} @Override protected void paintComponent(Graphics g){ Graphics2D g2=(Graphics2D)g.create(); g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING,RenderingHints.VALUE_ANTIALIAS_ON); g2.setColor(WHITE); g2.fillRoundRect(0,0,getWidth()-1,getHeight()-1,10,10); g2.setColor(BORDER); g2.drawRoundRect(0,0,getWidth()-1,getHeight()-1,10,10); g2.dispose(); super.paintComponent(g);} }
    private static class IconPlaceholderTextField extends JTextField { private final String placeholder; private final Icon icon; IconPlaceholderTextField(String placeholder, Icon icon){this.placeholder=placeholder;this.icon=icon;setMargin(new Insets(0,0,0,12));} @Override public Insets getInsets(){return new Insets(0,48,0,12);} @Override public Insets getInsets(Insets insets){insets.top=0;insets.left=48;insets.bottom=0;insets.right=12;return insets;} @Override protected void paintComponent(Graphics g){super.paintComponent(g); Graphics2D g2=(Graphics2D)g.create(); g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING,RenderingHints.VALUE_ANTIALIAS_ON); if(icon!=null)icon.paintIcon(this,g2,12,(getHeight()-icon.getIconHeight())/2); if(getText().isEmpty()){g2.setColor(MUTED);g2.setFont(getFont());int y=getHeight()/2+g2.getFontMetrics().getAscent()/2-2;g2.drawString(placeholder,48,y);} g2.dispose();} }
