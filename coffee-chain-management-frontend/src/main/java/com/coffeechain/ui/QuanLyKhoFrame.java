...
public class QuanLyKhoFrame extends JFrame {
    ...
    private final JTextField searchField = new IconPlaceholderTextField("Tìm kho", new FlatSVGIcon("icons/tim.svg", 18, 18));
...
//    private final JLabel selectedIdLabel = new JLabel("Chưa chọn kho");
    ...
    private void buildHeader() {
        ...
        root.add(title);
        JLabel subtitle = new JLabel("Theo dõi kho tổng, kho chi nhánh, trạng thái hoạt động và liên kết chi nhánh");
    ...
    private void buildToolbar() {
        ...
        addLabel(card, "Loại kho", 430, 12, 120, 20);
        addCombo(card, filterTypeCombo, 430, 40, 200, 36);
...
//        card.add(selectedIdLabel);
        ...
        new SwingWorker<WarehouseLookupDto, Void>() {
            ...
            protected void done() {
                try {
                    ...
                    warehouseTypes = lookup.getWarehouseTypes() == null ? new ArrayList<>() : lookup.getWarehouseTypes();
                    ...
                    JOptionPane.showMessageDialog(QuanLyKhoFrame.this, unwrapMessage(ex), "Lỗi", JOptionPane.ERROR_MESSAGE);
    ...
    private void populateTable() {
        ...
        for (WarehouseDto w : warehouses) {
            tableModel.addRow(new Object[]{w.getMaKho(), valueOrDash(w.getTenKho()), typeLabel(w.getLoaiKho()), valueOrDash(w.getTenChiNhanh()), statusText(w.getTrangThai())});
        }
    }
...
//        selectedIdLabel.setText("Đang sửa kho #" + selectedWarehouseId);
    ...
    private void saveWarehouse() {
        ...
        new SwingWorker<WarehouseDto, Void>() {
            ...
            protected WarehouseDto doInBackground() throws Exception {
                return createMode ? apiClient.createWarehouse(request) : apiClient.updateWarehouse(selectedWarehouseId, request);
            }
    ...
    private void deactivateWarehouse() {
        ...
        int confirm = JOptionPane.showConfirmDialog(this, "Bạn muốn chuyển kho đang chọn sang trạng thái ngưng hoạt động?", "Ngưng hoạt động kho", JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE);
...
//        selectedIdLabel.setText("Chưa chọn kho");
    ...
    private static RoundedButton secondaryButton(String text) {
        RoundedButton b = new RoundedButton(text).background(Color.decode("#B9B9B9")).hover(Color.decode("#A8A8A8")).radius(10);
