...
/**
 * Menu con của nhóm quản trị hệ thống.
 * Từ đây user có thể mở màn phân quyền hoặc màn tạo tài khoản nếu có quyền tương ứng.
 */
public class QuanTriHeThongFrame extends JFrame {
    ...
    private void buildMenuCards() {
        ...
        if (PermissionUtil.hasAny("ROLE:VIEW", "ROLE:CREATE", "ROLE:UPDATE", "ROLE:DELETE")) {
            items.add(new SystemMenuItem("Phân quyền & bảo mật", "Tạo role và\nquản lý quyền truy cập", ICON_PERMISSION,
                    () -> FrameNavigator.open(this, new PhanQuyenBaoMatFrame())));
        }

        if (PermissionUtil.hasAny("USER:CREATE", "USER:VIEW")) {
            items.add(new SystemMenuItem("Tạo tài khoản", "Tạo tài khoản\ncho nhân viên mới", ICON_CREATE_USER,
                    () -> FrameNavigator.open(this, new TaoTaiKhoanFrame())));
        }

        if (items.isEmpty()) {
            JLabel noPermission = new JLabel("<html><div style='text-align:center;'>Bạn chưa có quyền sử dụng chức năng quản trị hệ thống.<br/>Vui lòng liên hệ quản trị viên.</div></html>");
