...
public class QuanLyPOSFrame extends JFrame {
  ...
  private void buildMenuCards() {
    ...
    if (PermissionUtil.hasAny("ORDER:VIEW", "ORDER:CREATE", "ORDER:PAY")) {
      items.add(
          new QuanLyPOSFrame.SystemMenuItem(
              "Quản lý đơn hàng",
              "Tạo đơn hàng và\nthanh toán POS",
              ICON_PERMISSION,
    ...
    if (items.isEmpty()) {
      JLabel noPermission =
          new JLabel(
              "<html><div style='text-align:center;'>Bạn chưa có quyền sử dụng chức năng quản lý pos.<br/>Vui lòng liên hệ quản trị viên.</div></html>");
