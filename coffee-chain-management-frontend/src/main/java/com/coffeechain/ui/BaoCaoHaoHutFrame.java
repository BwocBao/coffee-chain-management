...
public class BaoCaoHaoHutFrame extends JFrame {
  ...
  private final JTextField reporterField =
  ...
  private void buildReportInfo() {
    ...
    addField(
        card, reporterField, 1110, 34, 190, 34, false, SessionManager.getCurrentUserDisplayName());
  }
  ...
  private void bindEvents() {
    ...
    wastageTypeCombo.addActionListener(
        e -> {
          if (!suppressEvents) {
            loadHistoryOnly();
          }
      ...
      protected void done() {
        try {
          ...
          ingredientCombo.setModel(
              new DefaultComboBoxModel<>(lookup.getIngredients().toArray(new OptionDto[0])));
          wastageTypeCombo.setModel(
              new DefaultComboBoxModel<>(buildFilterWastageTypes(lookup.getWastageTypes())));
          detailWastageTypeCombo.setModel(
              new DefaultComboBoxModel<>(lookup.getWastageTypes().toArray(new OptionDto[0])));
          ...
          JOptionPane.showMessageDialog(
              BaoCaoHaoHutFrame.this, unwrapMessage(ex), "Lỗi", JOptionPane.ERROR_MESSAGE);
        }
      }
    }.execute();
  }
  ...
  private void loadLotsAndHistory() {
    ...
    new SwingWorker<WastageScreenData, Void>() {
      ...
      protected WastageScreenData doInBackground() throws Exception {
        ...
        List<WastageDto> history =
            apiClient.searchWastages(maKho, maNguyenLieu, loaiHaoHut, null, null, null);
  ...
  private void saveWastage() {
    ...
    BigDecimal quantity =
        parsePositiveDecimal(quantityField.getText(), "Số lượng hao hụt phải lớn hơn 0");
    if (quantity == null) {
      return;
    }
    BigDecimal remaining =
        lot.getSoLuongConLai() == null ? BigDecimal.ZERO : lot.getSoLuongConLai();
      ...
      protected void done() {
        ...
        try {
          ...
          JOptionPane.showMessageDialog(
              ...
              "Đã tạo báo cáo hao hụt #"
                  + response.getMaPhieuHaoHut()
                  + "\nNguyên liệu: "
                  + response.getTenNguyenLieu()
                  + "\nSố lượng: "
                  + formatNumber(response.getSoLuongHaoHut())
                  + " "
                  + valueOrDash(response.getDonViTinh()),
              "Báo cáo hao hụt",
  ...
  private void updateLotFieldsFromCombo() {
    ...
    remainingField.setText(
        formatNumber(lot.getSoLuongConLai()) + " " + valueOrDash(lot.getDonViTinh()));
