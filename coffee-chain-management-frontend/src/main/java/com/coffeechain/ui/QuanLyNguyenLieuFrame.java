...
public class QuanLyNguyenLieuFrame extends JFrame {
    ...
    private final JTextField searchField = new IconPlaceholderTextField("Tìm nguyên liệu", new FlatSVGIcon("icons/tim.svg", 18, 18));
    ...
    private final DefaultTableModel tableModel = new DefaultTableModel(
            new Object[]{"Mã NL", "Tên nguyên liệu", "Đơn vị", "Ký hiệu", "Tồn tối thiểu", "Trạng thái"}, 0) {
        @Override
    ...
    private void buildHeader() {
        ...
        JLabel subtitle = new JLabel("Theo dõi danh mục nguyên liệu, đơn vị tính, mức tồn tối thiểu và trạng thái sử dụng");
        ...
        new SwingWorker<IngredientLookupDto, Void>() {
            ...
            protected void done() {
                try {
                    ...
                    units = lookup == null || lookup.getUnits() == null ? new ArrayList<>() : lookup.getUnits();
                    populateCombos();
                    loadIngredients();
                } catch (Exception ex) {
                    showStatus("Không tải được dữ liệu nguyên liệu: " + unwrapMessage(ex), true);
                    JOptionPane.showMessageDialog(QuanLyNguyenLieuFrame.this, unwrapMessage(ex), "Lỗi", JOptionPane.ERROR_MESSAGE);
    ...
    private void fillSelectedIngredient() {
        ...
        minimumStockField.setText(ingredient.getMucTonToiThieu() == null ? "" : ingredient.getMucTonToiThieu().stripTrailingZeros().toPlainString());
    ...
    private void saveIngredient() {
        ...
        new SwingWorker<IngredientDto, Void>() {
            ...
            protected IngredientDto doInBackground() throws Exception {
                return createMode ? apiClient.createIngredient(request) : apiClient.updateIngredient(selectedIngredientId, request);
            }
