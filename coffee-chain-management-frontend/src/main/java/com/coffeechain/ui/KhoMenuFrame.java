...
/**
 * Menu chức năng kho. Hiển thị các nghiệp vụ kho theo quyền như nhập, xuất, điều chuyển, kiểm kho,
...
public class KhoMenuFrame extends JFrame {
  ...
  private void buildMenuGrid() {
    List<KhoMenuItem> visibleItems = getMenuItems().stream().filter(this::canShow).toList();
  ...
  private List<KhoMenuItem> getMenuItems() {
    return List.of(
        new KhoMenuItem(
            "Nhập kho",
            "Tạo phiếu nhập và cập nhật tồn",
            "INVENTORY:IMPORT",
            "icons/menu-kho/nhap-kho.svg"),
        new KhoMenuItem(
            "Xuất kho",
            "Tạo phiếu xuất và trừ tồn",
            "INVENTORY:EXPORT",
            "icons/menu-kho/xuat-kho.svg"),
        new KhoMenuItem(
            "Điều chuyển kho",
            "Chuyển giữa các kho nguồn - đích",
            "INVENTORY:TRANSFER",
            "icons/menu-kho/dieu-chuyen-kho.svg"),
        new KhoMenuItem(
            "Kiểm kho", "So sánh tồn hệ thống", "STOCKTAKE:MANAGE", "icons/menu-kho/kiem-kho.svg"),
        new KhoMenuItem(
            "Báo cáo hao hụt",
            "Ghi nhận hư hỏng, hết hạn, thất thoát",
            "WASTAGE:CREATE",
            "icons/menu-kho/bao-cao-hao-hut.svg"),
        new KhoMenuItem(
            "Xem tồn kho",
            "Xem tồn hiện tại theo kho/nguyên liệu",
            "INVENTORY:VIEW",
            "icons/menu-kho/xem-ton-kho.svg"),
        new KhoMenuItem(
            "Theo dõi HSD",
            "Theo dõi lô, HSD và cảnh báo",
            "INVENTORY:VIEW",
            "icons/menu-kho/theo-doi-hsd.svg"),
        new KhoMenuItem(
            "Lịch sử",
            "Tra cứu nhập xuất điều chuyển",
            "INVENTORY:VIEW",
            "icons/menu-kho/tra-cuu-lich-su.svg"),
        new KhoMenuItem(
            "Quản lý thông tin kho",
            "Thêm, sửa, xóa thông tin kho",
            "WAREHOUSE:VIEW",
            "icons/menu-kho/quan-ly-kho.svg"),
        new KhoMenuItem(
            "Quản lý nguyên liệu",
            "Thêm, sửa, xóa nguyên liệu",
            "INGREDIENT:VIEW",
            "icons/menu-kho/quan-ly-nguyen-lieu.svg"),
        new KhoMenuItem(
            "Quản lý nhà cung cấp",
            "Thêm, sửa, ngưng hoạt động NCC",
            "SUPPLIER:VIEW",
            "icons/menu-kho/quan-ly-nha-cung-cap.svg"),
        new KhoMenuItem(
            "Quản lý đơn vị tính",
  ...
  private void openPlaceholder(KhoMenuItem item) {
    String message =
        "Bạn vừa chọn: "
            ...
            + "\n\n"
            + "Permission: "
            + (item.permission() == null ? "Chưa gắn quyền UI" : item.permission())
            + "\n\n"
  ...
  private record KhoMenuItem(
      String title, String description, String permission, String iconPath) {}
}
