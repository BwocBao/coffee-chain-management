...
public class ForgotPasswordFrame extends JFrame {
  ...
  private final PromptPasswordField confirmPasswordField =
      new PromptPasswordField("  Nhập lại mật khẩu mới");
  ...
  private void buildCard(JPanel card) {
    ...
    LoginFieldPanel newPasswordPanel =
        new LoginFieldPanel(new LockLineIcon(ICON_GRAY), newPasswordField, null);
    newPasswordPanel.setBounds(50, 330, 520, 58);
    card.add(newPasswordPanel);

    LoginFieldPanel confirmPasswordPanel =
        new LoginFieldPanel(new LockLineIcon(ICON_GRAY), confirmPasswordField, null);
    ...
    new Thread(
            () -> {
              try {
                ...
                SwingUtilities.invokeLater(
                    () -> showMessage("Ma khong hop le: " + ex.getMessage(), false));
              } finally {
                SwingUtilities.invokeLater(() -> setBusy(false, null));
              }
            })
    ...
    new Thread(
            () -> {
              try {
                ...
                SwingUtilities.invokeLater(
                    () -> showMessage("Doi mat khau that bai: " + ex.getMessage(), false));
              } finally {
                SwingUtilities.invokeLater(() -> setBusy(false, null));
              }
            })
