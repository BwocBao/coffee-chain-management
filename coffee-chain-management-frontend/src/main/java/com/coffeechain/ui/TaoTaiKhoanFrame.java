 ...
 * Màn tạo tài khoản.
 *
 * <p>Flow frontend: 1. Mở frame -> gọi GET /api/users/lookups để load role và chi nhánh. 2. Người
 * dùng nhập username, email, password. 3. Chọn branch và role. 4. Bấm Signup -> gọi POST
 * /api/users.
 *
 * <p>Luật phân quyền chính vẫn nằm ở backend: - ADMIN có thể tạo nhiều role. - QUAN_LY_CHI_NHANH
 * chỉ được tạo THU_NGAN trong chi nhánh của mình.
 */
    ...
    new SwingWorker<UserLookupResponse, Void>() {
      ...
      protected void done() {
        try {
          ...
          roleCombo.setModel(
              new DefaultComboBoxModel<>(response.getRoles().toArray(new OptionDto[0])));

          branchCombo.setModel(
              new DefaultComboBoxModel<>(response.getBranches().toArray(new OptionDto[0])));
          ...
          JOptionPane.showMessageDialog(
              thisFrame(), unwrapMessage(ex), "Tạo tài khoản", JOptionPane.ERROR_MESSAGE);
        }
      }
    }.execute();
  }
