 ...
 * <p>========================= LUỒNG TỔNG QUAN =========================
 *
 * <p>Khi mở màn hình: XuatKhoFrame() -> buildHeader() -> buildReceiptInfo() ->
 * buildIngredientPicker() -> buildTables() -> buildQuickInputRow() -> buildLotSection() ->
 * buildNoteAndFooter() -> bindEvents() -> loadLookups()
 *
 * <p>Trong đó: - Các hàm build... chỉ dùng để dựng giao diện. - bindEvents() dùng để gắn sự kiện
 * cho combo, checkbox, table. - loadLookups() là API đầu tiên được gọi để lấy dữ liệu kho và loại
 * xuất.
 *
 * <p>========================= LUỒNG API =========================
 *
 * <p>1. loadLookups() -> gọi GET /api/inventory/exports/lookups -> đổ dữ liệu vào warehouseCombo và
 * exportTypeCombo -> gọi tiếp loadStockForSelectedWarehouse()
 *
 * <p>2. loadStockForSelectedWarehouse() -> gọi GET /api/inventory/exports/stock?maKho=... -> đổ dữ
 * liệu vào bảng nguyên liệu bên trái -> đổ dữ liệu vào ingredientCombo
 *
 * <p>3. Nếu bật "Chọn lô thủ công": refreshLotsIfManual() -> gọi GET
 * /api/inventory/exports/lots?maKho=...&maNguyenLieu=... -> đổ dữ liệu vào bảng lô
 *
 * <p>4. Khi bấm "Lưu phiếu": saveReceipt() -> gom dữ liệu từ list lines -> tạo
 * CreateExportReceiptRequest -> gọi POST /api/inventory/exports
 *
 * <p>========================= LUỒNG SỰ KIỆN =========================
 *
 * <p>- Chọn kho: warehouseCombo ActionListener -> loadStockForSelectedWarehouse() ->
 * refreshLotsIfManual()
 *
 * <p>- Chọn nguyên liệu: ingredientCombo ActionListener -> refreshLotsIfManual()
 *
 * <p>- Tick chọn lô thủ công: manualLotCheckBox ActionListener -> handleManualModeChanged()
 *
 * <p>- Gõ số lượng ở bảng lô: lotTableModel TableModelListener -> updateManualQuantityFromLots()
 *
 * <p>- Chọn nguyên liệu ở bảng trái: ingredientTable ListSelectionListener ->
 * fillSelectedIngredientToDetail()
 *
 * <p>- Bấm "Thêm vào phiếu": addLine()
 *
 * <p>- Bấm "Lưu phiếu": saveReceipt()
 *
 * <p>Lưu ý: - lines là danh sách dữ liệu thật sẽ gửi backend. - exportTableModel chỉ là dữ liệu
 * hiển thị cho người dùng xem. - SwingWorker được dùng để gọi API ở background, tránh làm đứng giao
 * diện Swing.
 */
public class XuatKhoFrame extends JFrame {
  ...
  private final DefaultTableModel exportTableModel =
      new DefaultTableModel(
  ...
  private void buildReceiptInfo() {
    ...
    addField(
        card, creatorField, 860, 34, 220, 34, false, SessionManager.getCurrentUserDisplayName());
   ...
   *
   * <p>Đây là hàm quan trọng nhất để hiểu UI phản ứng ra sao.
   *
   * <p>1. warehouseCombo: Khi người dùng đổi kho xuất: - loadStockForSelectedWarehouse() để load
   * lại tồn kho của kho mới. - refreshLotsIfManual() nếu đang bật chọn lô thủ công.
   *
   * <p>2. ingredientCombo: Khi người dùng đổi nguyên liệu: - refreshLotsIfManual() nếu đang bật
   * chọn lô thủ công.
   *
   * <p>3. manualLotCheckBox: Khi tick/bỏ tick chọn lô thủ công: - handleManualModeChanged() xử lý
   * bật/tắt chế độ thủ công.
   *
   * <p>4. lotTableModel: Khi người dùng nhập số lượng ở cột "SL xuất": -
  ...
  private void bindEvents() {
    ...
    lotTableModel.addTableModelListener(
        e -> {
   ...
   *
   * <p>API gọi: GET /api/inventory/exports/lookups
   *
   * <p>Dữ liệu nhận: - Danh sách kho xuất. - Danh sách loại xuất.
   *
   * <p>Sau khi load xong: - Set dữ liệu cho warehouseCombo. - Set dữ liệu cho exportTypeCombo. -
   * Gọi loadStockForSelectedWarehouse() để load nguyên liệu còn tồn của kho đang chọn.
   *
   * <p>Vì gọi API có thể mất thời gian, dùng SwingWorker: - doInBackground(): chạy API ở background
   * thread. - done(): cập nhật UI sau khi API chạy xong.
   */
    ...
    new SwingWorker<InventoryExportLookupDto, Void>() {
      ...
      protected void done() {
        try {
          ...
          exportTypeCombo.setModel(
              new DefaultComboBoxModel<>(lookup.getExportTypes().toArray(new OptionDto[0])));
          ...
          JOptionPane.showMessageDialog(
              thisFrame(), unwrapMessage(ex), "Lỗi", JOptionPane.ERROR_MESSAGE);
        }
      }
    }.execute();
  }
   ...
   *
   * <p>Nếu đã có dòng xuất trong phiếu: - Hỏi người dùng có chắc muốn đổi chế độ không. - Nếu đồng
   * ý thì resetReceipt() để tránh dữ liệu cũ bị lệch.
   *
   * <p>Khi bật chọn lô thủ công: - Bật bảng lotTable. - Khóa quantityField vì số lượng sẽ được cộng
   * từ bảng lô. - Gọi refreshLotsIfManual() để load lô còn tồn.
   *
   * <p>Khi tắt chọn lô thủ công: - Tắt bảng lotTable. - Mở lại quantityField để người dùng tự nhập
   * số lượng tổng.
   */
  private void handleManualModeChanged() {
    ...
    quantityField.setToolTipText(
        manual ? "Số lượng xuất được tự động tính từ bảng lô bên dưới" : "0");
   ...
   *
   * <p>API gọi: GET /api/inventory/exports/lots?maKho=...&maNguyenLieu=...
   *
   * <p>Điều kiện chạy: - manualLotCheckBox phải đang được tick. - Đã chọn kho xuất. - Đã chọn
   * nguyên liệu.
   *
   * <p>Sau khi API trả về: - currentLots lưu danh sách lô gốc. - populateLotTable(currentLots) đổ
   * dữ liệu lên bảng lô.
   *
   * <p>currentLots rất quan trọng: - Dòng i trên bảng lotTable tương ứng với currentLots.get(i). -
   ...
   *
   * <p>Hàm này được gọi khi bấm nút "Thêm vào phiếu".
   *
   * <p>Luồng xử lý: 1. Lấy nguyên liệu đang chọn trong ingredientCombo. 2. Kiểm tra đang ở chế độ
   * FEFO hay chọn lô thủ công.
   *
   * <p>Nếu FEFO: - Đọc số lượng từ quantityField. - Không gửi danh sách lô. - Backend sẽ tự chọn lô
   * theo FEFO.
   *
   * <p>Nếu chọn lô thủ công: - Gọi collectManualLotSelections(). - Lấy danh sách lô người dùng
   * nhập. - Tự tính tổng số lượng từ các lô.
   *
   * <p>3. Đọc đơn giá xuất. 4. Tạo ExportLine. 5. Thêm ExportLine vào list lines. 6. Thêm một dòng
   ...
   *
   * <p>Hàm này chỉ được gọi khi manualLotCheckBox đang bật.
   *
   * <p>Kiểm tra: - Bảng lô có dữ liệu không. - Người dùng có nhập SL xuất cho ít nhất một lô không.
   * - SL xuất theo từng lô phải > 0. - SL xuất không được vượt quá số lượng còn lại của lô.
   *
   * <p>Kết quả trả về: - selections: danh sách maLoHang + soLuongXuat để gửi backend. -
   * totalQuantity: tổng số lượng xuất từ các lô.
   *
   * <p>Nếu dữ liệu không hợp lệ: - Hiện cảnh báo. - Return null để addLine() dừng lại.
   */
  private ManualLotResult collectManualLotSelections() {
    ...
    for (int i = 0; i < lotTableModel.getRowCount(); i++) {
      ...
      BigDecimal lotQty =
          parsePositiveDecimal(rawQty.toString(), "Số lượng xuất theo lô phải lớn hơn 0");
      ...
      if (lotQty.compareTo(available) > 0) {
        showWarning(
            "Số lượng xuất vượt quá số lượng còn lại của lô "
                + lot.getMaLoHang()
                + ". Còn lại: "
   ...
   *
   * <p>Hàm này được gọi tự động bởi TableModelListener trong bindEvents().
   *
   * <p>Ví dụ: - Lô 1 nhập 100 - Lô 2 nhập 200 => quantityField sẽ tự hiện 300
   *
   * <p>Lưu ý: - Hàm này chỉ cập nhật giao diện tạm thời. - Validate chính thức vẫn nằm trong
   * collectManualLotSelections().
   */
   ...
   *
   * <p>Hàm này được gọi khi bấm nút "Lưu phiếu".
   *
   * <p>Luồng xử lý: 1. Lấy kho xuất và loại xuất đang chọn. 2. Kiểm tra phiếu có ít nhất một dòng
   * trong list lines. 3. Tạo CreateExportReceiptRequest. 4. Duyệt list lines để tạo danh sách
   * CreateExportReceiptItemRequest.
   *
   * <p>Nếu đang chọn lô thủ công: - Mỗi item sẽ gửi thêm loHangXuat.
   *
   * <p>Nếu FEFO: - Không gửi loHangXuat. - Backend tự chọn lô.
   *
   * <p>5. Gọi API POST /api/inventory/exports bằng SwingWorker. 6. Thành công: - Hiện thông báo. -
   * resetReceipt(). - Nếu đang chọn lô thủ công thì refreshLotsIfManual(). 7. Thất bại: - Hiện lỗi
   * backend trả về.
   */
      ...
      protected void done() {
        ...
        try {
          ...
          JOptionPane.showMessageDialog(
              thisFrame(),
              "Đã tạo phiếu xuất #"
                  + receipt.getMaPhieuXuat()
                  + "\nKho: "
                  + receipt.getTenKho()
                  + "\nLoại xuất: "
                  + receipt.getLoaiXuat()
                  + "\nTổng giá trị: "
                  + numberFormat.format(nullToZero(receipt.getTongGiaTriXuat())),
              "Xuất kho",
   ...
   *
   * <p>API gọi: GET /api/inventory/exports/stock?maKho=...
   *
   * <p>Hàm này được gọi trong 2 trường hợp: 1. Sau khi loadLookups() xong. 2. Khi người dùng đổi
   * kho trong warehouseCombo.
   *
   * <p>Sau khi API trả về: - allIngredients lưu toàn bộ nguyên liệu còn tồn. - ingredientCombo được
   * cập nhật để chọn nguyên liệu. - ingredientTable bên trái được cập nhật để hiển thị danh sách
   * nguyên liệu.
   */
   ...
   *
   * <p>Đây là dữ liệu thật dùng để gửi backend.
   *
   * <p>ingredient: - nguyên liệu được chọn.
   *
   * <p>quantity: - tổng số lượng xuất.
   *
   * <p>price: - đơn giá xuất.
   *
   * <p>lotMode: - chỉ dùng để hiển thị trên bảng, ví dụ "FEFO" hoặc "Thủ công".
   *
   * <p>lotSelections: - danh sách lô cụ thể khi chọn lô thủ công. - nếu FEFO thì danh sách này
   * rỗng.
   */
  ...
  private record ManualLotResult(
      List<ExportLotSelectionRequest> selections, BigDecimal totalQuantity) {}
