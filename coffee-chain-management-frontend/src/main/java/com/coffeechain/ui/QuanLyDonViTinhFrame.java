...
public class QuanLyDonViTinhFrame extends JFrame {
  ...
  private final JTextField searchField =
      new IconPlaceholderTextField("Tìm đơn vị tính", new FlatSVGIcon("icons/tim.svg", 18, 18));
  ...
  private final DefaultTableModel tableModel =
      new DefaultTableModel(new Object[] {"Mã ĐVT", "Tên đơn vị tính", "Ký hiệu"}, 0) {
        @Override
  ...
  private void buildHeader() {
    ...
    JLabel subtitle =
        new JLabel(
            "Thiết lập đơn vị đo lường cho nguyên liệu như gram, ml, chai, gói và các ký hiệu liên quan");
  ...
  private void buildTable() {
    ...
    table
        ...
        .addListSelectionListener(
            e -> {
              if (!e.getValueIsAdjusting()) fillSelectedUnit();
            });
  ...
  private void loadUnits() {
    ...
    new SwingWorker<List<UnitDto>, Void>() {
      @Override
      protected List<UnitDto> doInBackground() throws Exception {
        return apiClient.searchUnits(keyword);
      }
      ...
      protected void done() {
        try {
          ...
          JOptionPane.showMessageDialog(
              QuanLyDonViTinhFrame.this, unwrapMessage(ex), "Lỗi", JOptionPane.ERROR_MESSAGE);
        } finally {
          loading = false;
  ...
  private void populateTable() {
    ...
    for (UnitDto unit : units)
      tableModel.addRow(
          new Object[] {
            unit.getMaDonViTinh(),
            valueOrDash(unit.getTenDonViTinh()),
            valueOrDash(unit.getKyHieu())
          });
  }
  ...
  private void saveUnit() {
    ...
    new SwingWorker<UnitDto, Void>() {
      @Override
      protected UnitDto doInBackground() throws Exception {
        return createMode
            ? apiClient.createUnit(request)
            : apiClient.updateUnit(selectedUnitId, request);
      }
  ...
  private UnitRequest buildRequest() {
    ...
    if (name.isBlank()) {
      showWarning("Vui lòng nhập tên đơn vị tính");
      nameField.requestFocusInWindow();
      return null;
    }
    if (symbol.isBlank()) {
      showWarning("Vui lòng nhập ký hiệu đơn vị tính");
  ...
  private void deleteUnit() {
    if (selectedUnitId == null) {
      showWarning("Vui lòng chọn đơn vị tính cần xóa");
      return;
    }
    int confirm =
        JOptionPane.showConfirmDialog(
            this,
            "Chỉ xóa được đơn vị tính chưa được nguyên liệu sử dụng. Bạn muốn xóa?",
            "Xóa đơn vị tính",
  ...
  private void showWarning(String msg) {
    JOptionPane.showMessageDialog(this, msg, "Quản lý đơn vị tính", JOptionPane.WARNING_MESSAGE);
  }
