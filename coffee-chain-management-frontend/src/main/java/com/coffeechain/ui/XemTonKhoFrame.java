...
public class XemTonKhoFrame extends JFrame {
    ...
    private final DefaultTableModel lotTableModel = new DefaultTableModel(
            new Object[]{"Mã lô", "Kho", "Nguyên liệu", "DVT", "Còn lại", "Trạng thái", "Hạn sử dụng", "Ngày tạo"}, 0
    ) {
        @Override
        public boolean isCellEditable(int row, int column) {
            return false;
    ...
    private void buildHeader() {
        ...
        JLabel subtitle = new JLabel("Theo dõi tồn hiện tại, cảnh báo tồn thấp và danh sách lô theo FEFO");
    ...
    private void loadData(boolean refreshFilters) {
        ...
        new SwingWorker<StockScreenData, Void>() {
            ...
            protected StockScreenData doInBackground() throws Exception {
                PageResponseDto<InventoryDto> stock = apiClient.getStock(
                        maKho,
                        maNguyenLieu,
                        tuKhoa,
                        trangThai,
    ...
    private void populateFilters(List<InventoryDto> rows) {
        ...
        warehouseCombo.addItem(new FilterOption(null, "Tất cả kho"));
        warehouses.forEach((id, name) -> warehouseCombo.addItem(new FilterOption(String.valueOf(id), name)));
        selectOption(warehouseCombo, selectedWarehouse);

        ingredientCombo.removeAllItems();
        ingredientCombo.addItem(new FilterOption(null, "Tất cả nguyên liệu"));
        ingredients.forEach((id, name) -> ingredientCombo.addItem(new FilterOption(String.valueOf(id), name)));
