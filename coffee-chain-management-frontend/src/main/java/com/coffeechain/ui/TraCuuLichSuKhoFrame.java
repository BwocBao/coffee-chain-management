...
public class TraCuuLichSuKhoFrame extends JFrame {
    ...
    private final DefaultTableModel historyTableModel = new DefaultTableModel(
            new Object[]{"Mã", "Thời gian", "Kho", "Nguyên liệu", "DVT", "Lô", "Loại giao dịch", "Chứng từ", "Thay đổi", "Trước", "Sau", "Người thao tác"}, 0
    ) {
        @Override
    ...
    private void buildHeader() {
        ...
        JLabel subtitle = new JLabel("Theo dõi nhật ký nhập, xuất, điều chuyển, hao hụt, bán hàng và điều chỉnh kiểm kho");
    ...
    private void loadData(boolean loadLookups) {
        ...
        new SwingWorker<HistoryScreenData, Void>() {
            ...
            protected HistoryScreenData doInBackground() throws Exception {
                InventoryHistoryLookupDto lookups = loadLookups || !lookupsLoaded ? apiClient.getLookups() : null;
                List<InventoryHistoryDto> history = apiClient.searchHistory(
                        maKho,
                        maNguyenLieu,
                        maLoHang,
                        loaiGiaoDich,
                ...
                List<InventoryHistorySummaryDto> summary = apiClient.getSummary(
                        maKho,
                        maNguyenLieu,
                        dates.fromDateTime(),
                        dates.toDateTime()
    ...
    private void populateCombo(JComboBox<FilterOption> combo, String allLabel, List<OptionDto> options, boolean useId) {
        FilterOption selected = (FilterOption) combo.getSelectedItem();
        combo.removeAllItems();
        combo.addItem(new FilterOption(null, null, allLabel));
        if (options != null) {
            for (OptionDto option : options) {
                combo.addItem(new FilterOption(
                        useId ? option.getId() : null,
                        useId ? null : option.getCode(),
                        option.getName()
                ));
            }
        }
    ...
    private void updateSummary(List<InventoryHistorySummaryDto> summaryRows, List<InventoryHistoryDto> historyRows) {
        ...
        if (summaryRows != null) {
            for (InventoryHistorySummaryDto row : summaryRows) {
                ...
                totalExport = totalExport.add(defaultDecimal(row.getTongXuat()))
                        .add(defaultDecimal(row.getTongHaoHut()))
                        .add(defaultDecimal(row.getTongBanHangTruKho()));
                totalTransfer = totalTransfer.add(defaultDecimal(row.getTongDieuChuyenVao()))
    ...
    private QueryDates parseDates() {
        ...
        if (from != null && to != null && from.isAfter(to)) {
            JOptionPane.showMessageDialog(this, "Từ ngày không được lớn hơn đến ngày", "Lịch sử kho", JOptionPane.WARNING_MESSAGE);
            return null;
        }
        return new QueryDates(
                from == null ? null : from + "T00:00:00",
                to == null ? null : to + "T23:59:59"
        );
    }
    ...
    private LocalDate parseOptionalDate(String raw, String label) {
        ...
        try {
            ...
            JOptionPane.showMessageDialog(this, label + " phải có dạng yyyy-MM-dd", "Lịch sử kho", JOptionPane.WARNING_MESSAGE);
    ...
    private void addField(JPanel parent, JTextField field, int x, int y, int w, int h, String tooltip) {
    ...
    private Color transactionColor(String label) {
        ...
        if (label.contains("Xuất") || label.contains("ra") || label.contains("Hao") || label.contains("Bán")) return DANGER;
