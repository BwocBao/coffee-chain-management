...
/**
 * Màn hình đăng nhập đầu tiên của app. Sau khi login thành công sẽ lưu session và mở {@link
 * MenuTongFrame}.
 */
public class LoginFrame extends JFrame {
  ...
  private void buildCard(JPanel card) {
    ...
    LoginFieldPanel usernamePanel =
        new LoginFieldPanel(new UserLineIcon(ICON_GRAY), usernameField, null);
    usernamePanel.setBounds(56, 215, 539, 74);
    card.add(usernamePanel);

    JButton eyeButton = createEyeButton();
    LoginFieldPanel passwordPanel =
        new LoginFieldPanel(new LockLineIcon(ICON_GRAY), passwordField, eyeButton);
