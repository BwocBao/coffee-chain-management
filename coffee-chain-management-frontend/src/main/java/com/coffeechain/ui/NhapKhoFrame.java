 ...
 *
 * Bo cuc bam theo file SVG: header, card thong tin, tim nguyen lieu, 2 bang,
 * hang nhap nhanh, ghi chu va cum nut luu/huy.
 */
public class NhapKhoFrame extends JFrame {
    ...
    private final DefaultTableModel ingredientTableModel = new DefaultTableModel(
            new Object[]{"Mã NL", "Tên nguyên liệu", "DVT"}, 0
    ) {
        @Override
    ...
    private final DefaultTableModel importTableModel = new DefaultTableModel(
            new Object[]{"Mã NL", "Tên nguyên liệu", "Hạn sử dụng", "Số lượng", "Đơn giá", "Thành tiền"}, 0
    ) {
        @Override
        public boolean isCellEditable(int row, int column) {
            return false;
...
//        card.add(statusLabel);
    ...
    private void buildQuickInputRow() {
        ...
        addField(detailCard, expiryField, 930, 34, 140, 30, true, EXPIRY_PLACEHOLDER, new ClockLineIcon());
        ...
        new SwingWorker<InventoryLookupDto, Void>() {
            ...
            protected void done() {
                try {
                    ...
                    supplierCombo.setModel(new DefaultComboBoxModel<>(
                            lookup.getSuppliers().toArray(new OptionDto[0])
                    ));
                    ...
                    JOptionPane.showMessageDialog(thisFrame(), unwrapMessage(ex), "Loi", JOptionPane.ERROR_MESSAGE);
                }
            }
        }.execute();
    }
    ...
    private void addLine() {
        ...
        BigDecimal quantity = parsePositiveDecimal(quantityField.getText(), "So luong nhap phai lon hon 0");
        if (quantity == null) {
            return;
        }
            ...
            protected void done() {
                ...
                try {
                    ...
                    JOptionPane.showMessageDialog(
                            thisFrame(),
                            "Da tao phieu nhap #" + receipt.getMaPhieuNhap()
                            + "\nKho: " + receipt.getTenKho()
                            + "\nNCC: " + receipt.getTenNhaCungCap()
                            + "\nTong tien: " + numberFormat.format(receipt.getTongTien()),
                            "Nhap kho",
