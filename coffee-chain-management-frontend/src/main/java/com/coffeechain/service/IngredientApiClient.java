...
public class IngredientApiClient extends ApiClientSupport {
  ...
  public List<IngredientDto> searchIngredients(String keyword, String status, Long unitId)
    ...
    String url =
        ApiConfig.INGREDIENTS_URL + (params.isBlank() ? "" : "?" + stripLeadingAmpersand(params));
    ...
    BaseResponse<List<IngredientDto>> baseResponse =
        readBaseResponse(response, new TypeReference<BaseResponse<List<IngredientDto>>>() {});
  ...
  public IngredientLookupDto getLookups() throws IOException, InterruptedException {
    ...
    HttpResponse<String> response = send(request);
    BaseResponse<IngredientLookupDto> baseResponse =
        readBaseResponse(response, new TypeReference<BaseResponse<IngredientLookupDto>>() {});
    return extractData(baseResponse);
  }

  public IngredientDto createIngredient(IngredientRequest requestBody)
      throws IOException, InterruptedException {
    HttpRequest request =
        HttpRequest.newBuilder()
    ...
    HttpResponse<String> response = send(request);
    BaseResponse<IngredientDto> baseResponse =
        readBaseResponse(response, new TypeReference<BaseResponse<IngredientDto>>() {});
    return extractData(baseResponse);
  }

  public IngredientDto updateIngredient(Long id, IngredientRequest requestBody)
      throws IOException, InterruptedException {
    HttpRequest request =
        HttpRequest.newBuilder()
