...
public class BranchApiClient extends ApiClientSupport {
  ...
  public List<BranchDto> searchBranches(String keyword, String status)
      throws IOException, InterruptedException {
    String params =
        optionalParamWithoutPrefix("keyword", keyword) + optionalParam("status", status);
    String url =
        ApiConfig.BRANCHES_URL + (params.isBlank() ? "" : "?" + stripLeadingAmpersand(params));
    HttpRequest request =
    ...
    BaseResponse<List<BranchDto>> baseResponse =
        readBaseResponse(response, new TypeReference<BaseResponse<List<BranchDto>>>() {});
  ...
  public BranchStatisticsDto getStatistics() throws IOException, InterruptedException {
    HttpRequest request =
        HttpRequest.newBuilder()
            .uri(URI.create(ApiConfig.BRANCH_STATISTICS_URL))
            .header("Authorization", bearerToken())
            .GET()
            .build();
    HttpResponse<String> response = send(request);
    BaseResponse<BranchStatisticsDto> baseResponse =
        readBaseResponse(response, new TypeReference<BaseResponse<BranchStatisticsDto>>() {});
    return extractData(baseResponse);
  }

  public BranchDto createBranch(BranchRequest requestBody)
      throws IOException, InterruptedException {
    HttpRequest request =
        HttpRequest.newBuilder()
            ...
            .build();
    HttpResponse<String> response = send(request);
    BaseResponse<BranchDto> baseResponse =
        readBaseResponse(response, new TypeReference<BaseResponse<BranchDto>>() {});
    return extractData(baseResponse);
  }

  public BranchDto updateBranch(Long id, BranchRequest requestBody)
      throws IOException, InterruptedException {
    HttpRequest request =
        HttpRequest.newBuilder()
