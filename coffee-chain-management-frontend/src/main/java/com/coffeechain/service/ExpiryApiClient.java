...
public class ExpiryApiClient extends ApiClientSupport {
    ...
    public ExpiryLookupDto getLookups() throws IOException, InterruptedException {
        ...
        BaseResponse<ExpiryLookupDto> baseResponse = readBaseResponse(
                response,
                new TypeReference<BaseResponse<ExpiryLookupDto>>() {}
        );
    ...
    public List<ExpiryLotDto> searchLots(
        ...
        String url = ApiConfig.INVENTORY_EXPIRY_LOTS_URL
        ...
        HttpResponse<String> response = send(request);
        BaseResponse<List<ExpiryLotDto>> baseResponse = readBaseResponse(
                response,
                new TypeReference<BaseResponse<List<ExpiryLotDto>>>() {}
        );
        return extractData(baseResponse);
    }

    public ExpiryStatisticsDto getStatistics(Long maKho, Integer warningDays) throws IOException, InterruptedException {
        String params = optionalParam("maKho", maKho) + optionalParam("warningDays", warningDays);
        String url = ApiConfig.INVENTORY_EXPIRY_STATISTICS_URL
                + (params.isBlank() ? "" : "?" + params.substring(1));
        ...
        HttpResponse<String> response = send(request);
        BaseResponse<ExpiryStatisticsDto> baseResponse = readBaseResponse(
                response,
                new TypeReference<BaseResponse<ExpiryStatisticsDto>>() {}
        );
        return extractData(baseResponse);
    }
    ...
    public ExpiryRefreshDto refreshExpiredLots() throws IOException, InterruptedException {
        ...
        HttpResponse<String> response = send(request);
        BaseResponse<ExpiryRefreshDto> baseResponse = readBaseResponse(
                response,
                new TypeReference<BaseResponse<ExpiryRefreshDto>>() {}
        );
        return extractData(baseResponse);
    }
