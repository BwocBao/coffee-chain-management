...
public class SupplierApiClient extends ApiClientSupport {
  ...
  public List<SupplierDto> searchSuppliers(String keyword)
    ...
    BaseResponse<List<SupplierDto>> baseResponse =
        readBaseResponse(response, new TypeReference<BaseResponse<List<SupplierDto>>>() {});
    List<SupplierDto> data = extractData(baseResponse);
    return data == null ? new ArrayList<>() : data;
  }

  public SupplierDto createSupplier(SupplierRequest requestBody)
      throws IOException, InterruptedException {
    HttpRequest request =
        HttpRequest.newBuilder()
    ...
    HttpResponse<String> response = send(request);
    BaseResponse<SupplierDto> baseResponse =
        readBaseResponse(response, new TypeReference<BaseResponse<SupplierDto>>() {});
    return extractData(baseResponse);
  }

  public SupplierDto updateSupplier(Long id, SupplierRequest requestBody)
      throws IOException, InterruptedException {
    HttpRequest request =
        HttpRequest.newBuilder()
