...
public class UnitApiClient extends ApiClientSupport {
  ...
  public List<UnitDto> searchUnits(String keyword) throws IOException, InterruptedException {
    ...
    BaseResponse<List<UnitDto>> baseResponse =
        readBaseResponse(response, new TypeReference<BaseResponse<List<UnitDto>>>() {});
  ...
  public UnitDto createUnit(UnitRequest requestBody) throws IOException, InterruptedException {
    HttpRequest request =
        HttpRequest.newBuilder()
            ...
            .build();
    HttpResponse<String> response = send(request);
    BaseResponse<UnitDto> baseResponse =
        readBaseResponse(response, new TypeReference<BaseResponse<UnitDto>>() {});
    return extractData(baseResponse);
  }

  public UnitDto updateUnit(Long id, UnitRequest requestBody)
