...
@Tag(
    name = "Theo doi han su dung lo hang",
    description =
        "Nhom API Theo doi han su dung lo hang. Swagger mo ta quyen can co, request, response va luong su dung chinh.")
@RestController
...
public class ExpiryController {
  ...
  @ApiResponses({
    ...
    @ApiResponse(
        responseCode = "403",
        description = "Không có quyền xem tồn kho",
        content = @Content)
  })
  @GetMapping("/lookups")
  public ResponseEntity<BaseResponse<ExpiryLookupResponse>> getLookups(
    ...
    return ResponseEntity.ok(
        BaseResponse.ok(
            "Lấy dữ liệu theo dõi hạn sử dụng thành công", expiryService.getLookups(user)));
  }
  ...
  public ResponseEntity<BaseResponse<ExpiryLotResponse>> getLotById(
    ...
    return ResponseEntity.ok(
        BaseResponse.ok("Lấy chi tiết lô hàng thành công", expiryService.getLotById(user, id)));
  }
  ...
  @ApiResponses({
    ...
    @ApiResponse(
        responseCode = "403",
        description = "Không có quyền cập nhật tồn kho",
        content = @Content)
  })
  @PostMapping("/refresh")
  public ResponseEntity<BaseResponse<ExpiryRefreshResponse>> refreshExpiredLots(
    ...
    return ResponseEntity.ok(
        BaseResponse.ok("Rà soát hạn sử dụng thành công", expiryService.refreshExpiredLots()));
  }
}
