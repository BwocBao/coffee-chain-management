...
@Tag(
    name = "Bao cao hao hut",
    description =
        "Nhom API Bao cao hao hut. Swagger mo ta quyen can co, request, response va luong su dung chinh.")
@RestController
...
public class WastageController {
  ...
  public ResponseEntity<BaseResponse<WastageLookupResponse>> getLookups(
    ...
    return ResponseEntity.ok(
        BaseResponse.ok("Lấy dữ liệu báo cáo hao hụt thành công", wastageService.getLookups(user)));
  }
  ...
  @ApiResponses({
    ...
    @ApiResponse(
        responseCode = "403",
        description = "Không có quyền xem hao hụt",
        content = @Content)
  })
  @GetMapping
    ...
    return ResponseEntity.ok(
        BaseResponse.ok(
            ...
            wastageService.searchWastages(
                user, maKho, maNguyenLieu, loaiHaoHut, fromDate, toDate, keyword)));
  }
  ...
  public ResponseEntity<BaseResponse<WastageResponse>> getById(
    ...
    return ResponseEntity.ok(
        BaseResponse.ok("Lấy chi tiết phiếu hao hụt thành công", wastageService.getById(user, id)));
  }
  ...
  public ResponseEntity<BaseResponse<WastageResponse>> createWastage(
    ...
    return ResponseEntity.ok(
        BaseResponse.created(
            "Tạo báo cáo hao hụt thành công", wastageService.createWastage(user, request)));
  }
}
