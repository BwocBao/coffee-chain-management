...
@Tag(
    name = "Kiem kho",
    description =
        "Nhom API Kiem kho. Swagger mo ta quyen can co, request, response va luong su dung chinh.")
@RestController
...
public class StocktakeController {
  ...
  public ResponseEntity<BaseResponse<StocktakeLookupResponse>> getLookups(
    ...
    return ResponseEntity.ok(
        BaseResponse.ok("Lấy dữ liệu kiểm kho thành công", stocktakeService.getLookups(user)));
  }

  @Operation(
      summary = "Lấy số lượng hệ thống để lập phiếu kiểm kho",
      description =
          "Frontend gọi API này sau khi chọn kho hoặc nguyên liệu để lấy danh sách lô còn tồn.")
  ...
  @ApiResponses({
    ...
    @ApiResponse(
        responseCode = "403",
        description = "Không có quyền xem kiểm kho",
        content = @Content)
  })
  @GetMapping
  ...
  public ResponseEntity<BaseResponse<StocktakeResponse>> getById(
    ...
    return ResponseEntity.ok(
        BaseResponse.ok(
            "Lấy chi tiết phiếu kiểm kho thành công", stocktakeService.getById(user, id)));
  }
  ...
  public ResponseEntity<BaseResponse<StocktakeResponse>> createStocktake(
    ...
    return ResponseEntity.ok(
        BaseResponse.created(
            "Tạo phiếu kiểm kho thành công", stocktakeService.createStocktake(user, request)));
  }
  ...
  public ResponseEntity<BaseResponse<StocktakeResponse>> completeStocktake(
    ...
    return ResponseEntity.ok(
        BaseResponse.ok(
            "Hoàn tất kiểm kho thành công", stocktakeService.completeStocktake(user, id)));
  }
  ...
  public ResponseEntity<BaseResponse<StocktakeResponse>> cancelStocktake(
    ...
    return ResponseEntity.ok(
        BaseResponse.ok(
            "Hủy phiếu kiểm kho thành công", stocktakeService.cancelStocktake(user, id)));
  }
}
