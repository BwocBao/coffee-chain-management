...
@Tag(
    ...
    description =
        "Nhom API Quan ly chi nhanh. Swagger mo ta quyen can co, request, response va luong su dung chinh.")
@RestController
...
public class BranchController {
  ...
  @ApiResponses({
    ...
    @ApiResponse(
        responseCode = "403",
        description = "Không có quyền xem chi nhánh",
        content = @Content)
  })
  @GetMapping
  public ResponseEntity<BaseResponse<List<BranchResponse>>> searchBranches(
    ...
    return ResponseEntity.ok(
        BaseResponse.ok(
            "Lấy danh sách chi nhánh thành công", branchService.searchBranches(keyword, status)));
  }

  @Operation(
      summary = "Lấy thống kê chi nhánh",
      description =
          "Trả về tổng số chi nhánh, số chi nhánh đang hoạt động, đã đóng và chi nhánh nhiều nhân viên nhất.")
  ...
  public ResponseEntity<BaseResponse<BranchStatisticsResponse>> getStatistics(
    ...
    return ResponseEntity.ok(
        BaseResponse.ok("Lấy thống kê chi nhánh thành công", branchService.getStatistics()));
  }
  ...
  public ResponseEntity<BaseResponse<BranchResponse>> getBranchById(
    ...
    return ResponseEntity.ok(
        BaseResponse.ok("Lấy chi tiết chi nhánh thành công", branchService.getBranchById(id)));
  }
  ...
  public ResponseEntity<BaseResponse<BranchResponse>> createBranch(
    ...
    return ResponseEntity.ok(
        BaseResponse.created("Tạo chi nhánh thành công", branchService.createBranch(request)));
  }
  ...
  public ResponseEntity<BaseResponse<BranchResponse>> updateBranch(
    ...
    return ResponseEntity.ok(
        BaseResponse.ok("Cập nhật chi nhánh thành công", branchService.updateBranch(id, request)));
  }
