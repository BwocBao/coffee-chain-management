...
@Tag(
    ...
    description =
        "API quản lý vai trò, danh sách quyền và gán quyền cho vai trò. Chỉ ADMIN được sử dụng nhóm API này.")
@RestController
...
public class RbacController {
  ...
  public ResponseEntity<BaseResponse<RolePermissionResponse>> rolePermissions(
      @Parameter(hidden = true) @RequestHeader(value = "Authorization", required = false)
          String authHeader,
      @Parameter(description = "ID của vai trò cần xem quyền", example = "4", required = true)
          @PathVariable
          Long roleId) {
    authGuard.requireRole(authHeader, "ADMIN");
    return ResponseEntity.ok(
        BaseResponse.ok(
            "Lấy quyền của vai trò thành công", rbacService.findRolePermissions(roleId)));
  }
  ...
  @ApiResponses({
    ...
    @ApiResponse(
        responseCode = "403",
        description = "Không phải ADMIN hoặc không được sửa role này",
        content = @Content),
    @ApiResponse(responseCode = "404", description = "Không tìm thấy vai trò", content = @Content)
  })
  ...
  public ResponseEntity<BaseResponse<RolePermissionResponse>> updateRolePermissions(
      @Parameter(hidden = true) @RequestHeader(value = "Authorization", required = false)
          String authHeader,
      @Parameter(description = "ID của vai trò cần cập nhật quyền", example = "4", required = true)
          @PathVariable
          Long roleId,
    ...
    return ResponseEntity.ok(
        BaseResponse.ok(
            "Cập nhật phân quyền thành công", rbacService.updateRolePermissions(roleId, request)));
  }
}
