...
@Tag(
    name = "Tra cuu lich su giao dich kho",
    description =
        "Nhom API Tra cuu lich su giao dich kho. Swagger mo ta quyen can co, request, response va luong su dung chinh.")
@RestController
...
public class InventoryHistoryController {
  ...
  @Operation(
      ...
      description =
          "Trả về danh sách kho, nguyên liệu và loại giao dịch kho để frontend render bộ lọc.")
  @GetMapping("/lookups")
  public ResponseEntity<BaseResponse<InventoryHistoryLookupResponse>> getLookups(
    ...
    return ResponseEntity.ok(
        BaseResponse.ok(
            "Lấy dữ liệu tra cứu lịch sử kho thành công", historyService.getLookups(user)));
  }
  ...
  @ApiResponses({
    ...
    @ApiResponse(
        responseCode = "403",
        description = "Không có quyền xem lịch sử kho",
        content = @Content)
  })
  @GetMapping
    ...
    return ResponseEntity.ok(
        BaseResponse.ok(
            ...
            historyService.searchHistory(
                user, maKho, maNguyenLieu, maLoHang, loaiGiaoDich, fromDate, toDate, keyword)));
  }
  ...
  public ResponseEntity<BaseResponse<InventoryHistoryResponse>> getHistoryById(
    ...
    return ResponseEntity.ok(
        BaseResponse.ok(
            "Lấy chi tiết nhật ký kho thành công", historyService.getHistoryById(user, id)));
  }
