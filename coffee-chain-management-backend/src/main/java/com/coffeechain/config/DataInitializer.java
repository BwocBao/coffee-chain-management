    ...
    public void run(String... args) {
        ...
        for (Map.Entry<String, List<String>> entry : seed.entrySet()) {
            ...
            for (String action : entry.getValue()) {
                Long maQuyen = nguoiDungRepository.getOrCreatePermission(
                        maChucNang,
                        action,
                        permissionDisplayName(entry.getKey(), action)
                );
        ...
        if (!nguoiDungRepository.existsUser(defaultAdminUsername)) {
            ...
            nguoiDungRepository.createUser(defaultAdminUsername, hash, adminRoleId, null, defaultAdminEmail);
            System.out.println("Đã tạo user mặc định: " + defaultAdminUsername + " / " + defaultAdminPassword);
        }
    }
