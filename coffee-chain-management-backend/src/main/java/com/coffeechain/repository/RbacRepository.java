...
public class RbacRepository {
    ...
    public List<RoleResponse> findAllRoles() {
        ...
        return jdbcTemplate.query(sql, (rs, rowNum) ->
                new RoleResponse(rs.getLong("ma_vai_tro"), rs.getString("ten_vai_tro"))
        );
    }
    ...
    public Optional<RoleResponse> findRoleById(Long maVaiTro) {
        List<RoleResponse> rows = jdbcTemplate.query("""
                ...
                FROM VAITRO
                WHERE ma_vai_tro = ?
                """, (rs, rowNum) -> new RoleResponse(rs.getLong("ma_vai_tro"), rs.getString("ten_vai_tro")), maVaiTro);
    ...
    public Optional<RoleResponse> findRoleByName(String tenVaiTro) {
        List<RoleResponse> rows = jdbcTemplate.query("""
                ...
                """, (rs, rowNum) -> new RoleResponse(rs.getLong("ma_vai_tro"), rs.getString("ten_vai_tro")), tenVaiTro);
        return rows.stream().findFirst();
    }
    ...
    public Long createRole(String tenVaiTro) {
        ...
        return jdbcTemplate.queryForObject(
                "SELECT ma_vai_tro FROM VAITRO WHERE ten_vai_tro = ?",
                Long.class,
                tenVaiTro
        );
    }
    ...
    public List<PermissionResponse> findAllPermissions() {
        String sql = """
                ...
                ORDER BY
                    ...
                    END
                """;
        return jdbcTemplate.query(sql, (rs, rowNum) -> new PermissionResponse(
                rs.getLong("ma_quyen"),
    ...
    public Set<Long> findPermissionIdsByRoleId(Long maVaiTro) {
        List<Long> ids = jdbcTemplate.query("""
                ...
                WHERE ma_vai_tro = ?
                ORDER BY ma_quyen
                """, (rs, rowNum) -> rs.getLong("ma_quyen"), maVaiTro);
    ...
    public Set<String> findPermissionCodesByRoleId(Long maVaiTro) {
        List<String> codes = jdbcTemplate.query("""
                ...
                """, (rs, rowNum) -> rs.getString("ten_chuc_nang").toUpperCase() + ":" + rs.getString("hanh_dong").toUpperCase(), maVaiTro);
    ...
    public int countPermissionsByIds(Set<Long> ids) {
        ...
        Integer count = jdbcTemplate.queryForObject("SELECT COUNT(*) FROM QUYEN WHERE ma_quyen IN (" + placeholders + ")", Integer.class, ids.toArray());
    ...
    public void replaceRolePermissions(Long maVaiTro, Set<Long> permissionIds) {
        ...
        for (Long permissionId : permissionIds) {
            jdbcTemplate.update("INSERT INTO VAITRO_QUYEN (ma_vai_tro, ma_quyen) VALUES (?, ?)", maVaiTro, permissionId);
        }
    }
}
