...
public class RecipeRepository {
  ...
  public RecipeRepository(JdbcTemplate jdbcTemplate) {
    ...
    this.productInsert =
        new SimpleJdbcInsert(jdbcTemplate)
            ...
            .usingColumns("ten_san_pham", "hinh_anh", "gia_ban_hien_tai", "trang_thai");

    this.formulaInsert =
        new SimpleJdbcInsert(jdbcTemplate)
            .withTableName("CONGTHUC_SANPHAM")
            .usingGeneratedKeyColumns("ma_cong_thuc")
            .usingColumns("ma_san_pham", "ma_nguyen_lieu", "so_luong_can");
  }
  ...
  public RecipeSummaryResponse findRecipeSummaryById(Long maSanPham) {
    ...
    List<RecipeSummaryResponse> rows =
        jdbcTemplate.query(
            sql,
            (rs, rowNum) ->
                new RecipeSummaryResponse(
  ...
  public String findProductImage(Long maSanPham) {
    ...
    List<String> rows =
        jdbcTemplate.query(sql, (rs, rowNum) -> rs.getString("hinh_anh"), maSanPham);
    return rows.isEmpty() ? null : rows.get(0);
  }
  ...
  public Long insertProduct(
      String tenSanPham, String hinhAnh, BigDecimal giaBanHienTai, String trangThai) {
  ...
  public void updateProduct(
    ...
    jdbcTemplate.update(sql, tenSanPham, hinhAnh, giaBanHienTai, trangThai, maSanPham);
  }

  public Long insertFormulaItem(Long maSanPham, Long maNguyenLieu, BigDecimal soLuongCan) {
    Map<String, Object> params = new LinkedHashMap<>();
  ...
  public void deleteFormulaByProduct(Long maSanPham) {
    jdbcTemplate.update("DELETE FROM CONGTHUC_SANPHAM WHERE ma_san_pham = ?", maSanPham);
  }

  public void updateProductStatus(Long maSanPham, String status) {
    jdbcTemplate.update(
        "UPDATE SANPHAM SET trang_thai = ? WHERE ma_san_pham = ?", status, maSanPham);
  }

  public boolean productExists(Long maSanPham) {
    Integer count =
        jdbcTemplate.queryForObject(
            "SELECT COUNT(*) FROM SANPHAM WHERE ma_san_pham = ?", Integer.class, maSanPham);
  ...
  public List<RecipeLookupResponse.IngredientOption> findIngredientOptions() {
    ...
    return jdbcTemplate.query(
        sql,
        (rs, rowNum) ->
            new RecipeLookupResponse.IngredientOption(
                rs.getLong("ma_nguyen_lieu"),
                rs.getString("ten_nguyen_lieu"),
