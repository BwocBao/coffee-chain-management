  ...
  public BankQrResponse createBankQr(Long maHoaDon) {
    ...
    try {
      CreatePaymentLinkResponse response =
          new PayOS(clientId, apiKey, checksumKey).paymentRequests().create(request);

      Long maThanhToan =
          posRepository.createPayOsPayment(
              maHoaDon,
              orderCode,
      ...
      PayOsPaymentRow saved =
          posRepository
              .findPayOsPaymentById(maThanhToan)
              .orElseThrow(
                  () ->
                      new AppException(
                          HttpStatus.INTERNAL_SERVER_ERROR,
                          "Khong doc duoc giao dich payOS vua tao"));
