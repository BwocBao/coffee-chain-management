...
public class RbacService {
    ...
    public RoleResponse createRole(CreateRoleRequest request) {
        if (request == null || request.getTenVaiTro() == null || request.getTenVaiTro().trim().isEmpty()) {
        ...
        if ("ADMIN".equals(tenVaiTro)) throw new AppException(HttpStatus.BAD_REQUEST, "Không được tạo trùng vai trò ADMIN");
        if (rbacRepository.findRoleByName(tenVaiTro).isPresent()) throw new AppException(HttpStatus.CONFLICT, "Vai trò đã tồn tại: " + tenVaiTro);
    ...
    public List<PermissionGroupResponse> findAllPermissionGroups() {
        ...
        for (PermissionResponse permission : rbacRepository.findAllPermissions()) {
            groups.computeIfAbsent(permission.getModule(), key -> new PermissionGroupResponse(key, moduleDisplayName(key)))
    ...
    @Transactional
    public RolePermissionResponse updateRolePermissions(Long maVaiTro, UpdateRolePermissionsRequest request) {
        RoleResponse role = findExistingRole(maVaiTro);
        if ("ADMIN".equalsIgnoreCase(role.getTenVaiTro())) {
            throw new AppException(HttpStatus.BAD_REQUEST, "Không được sửa quyền của vai trò ADMIN để tránh khóa hệ thống");
        }
        Set<Long> permissionIds = request == null || request.getPermissionIds() == null
                ? new LinkedHashSet<>()
                : new LinkedHashSet<>(request.getPermissionIds());
        if (!permissionIds.isEmpty() && rbacRepository.countPermissionsByIds(permissionIds) != permissionIds.size()) {
    ...
    private RoleResponse findExistingRole(Long maVaiTro) {
        ...
        return rbacRepository.findRoleById(maVaiTro)
                .orElseThrow(() -> new AppException(HttpStatus.NOT_FOUND, "Không tìm thấy vai trò"));
    }

    private String normalizeRoleCode(String value) {
        String noAccent = Normalizer.normalize(value.trim().toUpperCase(Locale.ROOT), Normalizer.Form.NFD).replaceAll("\\p{M}", "");
