  ...
  public void deleteSupplier(Long id) {
    ...
    if (supplierRepository.hasImportReceipt(id)) {
      throw new AppException(
          HttpStatus.BAD_REQUEST, "Nhà cung cấp đã có phiếu nhập, không thể xóa");
    }

    supplierRepository.deleteSupplier(id);
  }

  private void validateSupplierInput(String name, String phone, String email, String address) {
    ...
    if (name.length() > 150) {
      throw new AppException(
          HttpStatus.BAD_REQUEST, "Tên nhà cung cấp không được vượt quá 150 ký tự");
    }
