    ...
    public ImageUploadResponse uploadImage(MultipartFile file) {
        ...
        try {
            ...
            if (!response.getStatusCode().is2xxSuccessful() || response.getBody() == null) {
                throw new AppException(HttpStatus.BAD_GATEWAY, "Cloudinary khong tra ve ket qua upload hop le");
            }
            ...
            throw new AppException(HttpStatus.BAD_GATEWAY, "Upload anh len Cloudinary that bai: " + rootMessage(e));
        }
    }

    private void validateConfig() {
        if (isBlank(cloudName) || isBlank(apiKey) || isBlank(apiSecret)) {
            throw new AppException(HttpStatus.INTERNAL_SERVER_ERROR, "Chua cau hinh Cloudinary tren backend");
        }
    }
