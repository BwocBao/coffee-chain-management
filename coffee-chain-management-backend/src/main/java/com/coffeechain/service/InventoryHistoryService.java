...
public class InventoryHistoryService {
  ...
  public List<InventoryHistoryResponse> searchHistory(
    ...
    return historyRepository.searchHistory(
        allowedWarehouseId, maNguyenLieu, maLoHang, normalizedType, fromDate, toDate, keyword);
  }
  ...
  public InventoryHistoryResponse getHistoryById(SessionUser user, Long id) {
    ...
    return historyRepository.findHistoryById(id, forcedWarehouseId).stream()
        .findFirst()
  ...
  public List<InventoryHistorySummaryResponse> getSummary(
    ...
    return historyRepository.getSummary(allowedWarehouseId, maNguyenLieu, fromDate, toDate);
  }
  ...
  private Long resolveForcedWarehouseId(SessionUser user, Long requestedWarehouseId) {
    ...
    if (requestedWarehouseId != null && !requestedWarehouseId.equals(ownWarehouseId)) {
      throw new AppException(
          HttpStatus.FORBIDDEN, "Bạn chỉ được xem lịch sử kho của chi nhánh mình");
    }
  ...
  private List<InventoryHistoryLookupResponse.OptionDto> buildTransactionTypeOptions() {
    return List.of(
        new InventoryHistoryLookupResponse.OptionDto(
            null, "IMPORT", "Nhập kho", "Tăng tồn từ phiếu nhập"),
        new InventoryHistoryLookupResponse.OptionDto(
            null, "EXPORT", "Xuất kho", "Giảm tồn từ phiếu xuất"),
        new InventoryHistoryLookupResponse.OptionDto(
            null, "TRANSFER_IN", "Điều chuyển vào", "Tăng tồn do nhận điều chuyển"),
        new InventoryHistoryLookupResponse.OptionDto(
            null, "TRANSFER_OUT", "Điều chuyển ra", "Giảm tồn do xuất điều chuyển"),
        new InventoryHistoryLookupResponse.OptionDto(
            null, "WASTAGE", "Hao hụt", "Giảm tồn do hư hỏng, mất mát, hết hạn"),
        new InventoryHistoryLookupResponse.OptionDto(
            null, "SALE_DEDUCT", "Bán hàng trừ kho", "Giảm tồn do POS bán hàng"),
        new InventoryHistoryLookupResponse.OptionDto(
            null, "SALE_REVERSE", "Hoàn trừ kho", "Cộng lại tồn khi hoàn/hủy bán hàng"),
        new InventoryHistoryLookupResponse.OptionDto(
            null, "STOCKTAKE_ADJUST", "Điều chỉnh kiểm kho", "Tăng hoặc giảm tồn sau kiểm kho"));
  }
