...
public class PosInventoryDeductionService {
  ...
  public void completePaidOrderAndDeductStock(Long maHoaDon) {
    ...
    List<OrderDetailForDeductionRow> orderDetails =
        posRepository.findOrderDetailsForDeduction(maHoaDon);
    ...
    for (OrderDetailForDeductionRow detail : orderDetails) {
      ...
      for (RecipeLineRow recipe : recipeLines) {
        BigDecimal requiredQty = recipe.soLuongCan().multiply(BigDecimal.valueOf(detail.soLuong()));
  ...
  private void deductIngredientByFefo(
    ...
    for (LotRow lot : lots) {
      ...
      posRepository.createSaleDeduction(
          maHoaDon, maCtHoaDon, maKho, maNguyenLieu, lot.maLoHang(), soLuongMoiSp, deductQty);
