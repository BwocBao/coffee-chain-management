...
public class ExpiryService {
    ...
    public ExpiryLookupResponse getLookups(SessionUser user) {
        ...
        response.setStatuses(List.of(
                new ExpiryLookupResponse.OptionDto(null, STATUS_ACTIVE, "Đang hoạt động", "Lô còn sử dụng"),
                new ExpiryLookupResponse.OptionDto(null, STATUS_EXPIRED, "Đã hết hạn", "Lô quá hạn sử dụng"),
                new ExpiryLookupResponse.OptionDto(null, STATUS_USED_UP, "Đã dùng hết", "Lô không còn số lượng")
        ));

        response.setWarningLevels(List.of(
                new ExpiryLookupResponse.OptionDto(null, WARNING_DA_HET_HAN, "Đã hết hạn", "Lô đã quá hạn sử dụng"),
                new ExpiryLookupResponse.OptionDto(null, WARNING_SAP_HET_HAN, "Sắp hết hạn", "Lô còn hạn nhưng sắp hết hạn"),
                new ExpiryLookupResponse.OptionDto(null, WARNING_BINH_THUONG, "Bình thường", "Lô còn hạn dài"),
                new ExpiryLookupResponse.OptionDto(null, WARNING_KHONG_CO_HSD, "Không có hạn sử dụng", "Lô không nhập hạn sử dụng"),
                new ExpiryLookupResponse.OptionDto(null, WARNING_HET_HANG, "Hết hàng", "Lô đã dùng hết")
        ));
    ...
    public ExpiryLotResponse getLotById(SessionUser user, Long maLoHang) {
        ...
        return expiryRepository.findLotById(maLoHang, forcedWarehouseId)
                .stream()
                .findFirst()
                .orElseThrow(() -> new AppException(HttpStatus.NOT_FOUND, "Không tìm thấy lô hàng"));
    }

    public ExpiryStatisticsResponse getStatistics(
            SessionUser user,
            Long maKho,
            Integer warningDays
    ) {
    ...
    public ExpiryRefreshResponse refreshExpiredLots() {
        ...
        return new ExpiryRefreshResponse(
                "Đã rà soát và cập nhật trạng thái lô hết hạn",
                LocalDateTime.now()
        );
    }
    ...
    private Long resolveForcedWarehouseId(SessionUser user, Long requestedWarehouseId) {
        ...
        if (requestedWarehouseId != null && !requestedWarehouseId.equals(ownWarehouseId)) {
            throw new AppException(HttpStatus.FORBIDDEN, "Bạn chỉ được xem lô hàng thuộc kho của chi nhánh mình");
        }
    ...
    private Integer normalizeWarningDays(Integer warningDays) {
        ...
        if (warningDays < 1 || warningDays > 365) {
            throw new AppException(HttpStatus.BAD_REQUEST, "Số ngày cảnh báo phải nằm trong khoảng 1 đến 365");
        }
    ...
    private Integer normalizeDaysToExpire(Integer daysToExpire) {
        ...
        if (daysToExpire < 0 || daysToExpire > 365) {
            throw new AppException(HttpStatus.BAD_REQUEST, "Số ngày lọc hạn sử dụng phải nằm trong khoảng 0 đến 365");
        }
