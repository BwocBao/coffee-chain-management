...
public class UnitService {
  ...
  public void deleteUnit(Long id) {
    ...
    if (unitRepository.hasIngredient(id)) {
      throw new AppException(
          HttpStatus.BAD_REQUEST, "Đơn vị tính đã được nguyên liệu sử dụng, không thể xóa");
    }
  ...
  private void validateUnitInput(String name, String symbol) {
    ...
    if (name.length() > 50) {
      throw new AppException(
          HttpStatus.BAD_REQUEST, "Tên đơn vị tính không được vượt quá 50 ký tự");
    }
    ...
    if (symbol.length() > 20) {
      throw new AppException(
          HttpStatus.BAD_REQUEST, "Ký hiệu đơn vị tính không được vượt quá 20 ký tự");
    }
  }
