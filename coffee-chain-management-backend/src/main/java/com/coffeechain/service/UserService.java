...
public class UserService {
    ...
    public CreateUserLookupResponse getCreateUserLookups(SessionUser currentUser) {
        ...
        if (ROLE_QUAN_LY_CHI_NHANH.equals(currentRole)) {
            ...
            response.setRoles(nguoiDungRepository.findRoleOptionByName(ROLE_THU_NGAN)
                    .map(List::of)
                    .orElseThrow(() -> new AppException(HttpStatus.BAD_REQUEST, "Không tìm thấy vai trò THU_NGAN")));

            response.setBranches(nguoiDungRepository.findBranchOptionById(maChiNhanh)
                    .map(List::of)
                    .orElseThrow(() -> new AppException(HttpStatus.BAD_REQUEST, "Chi nhánh của quản lý không tồn tại hoặc không hoạt động")));
    ...
    public CreateUserResponse createUser(SessionUser currentUser, CreateUserRequest request) {
        ...
        Long maChiNhanh = resolveBranchForNewUser(
                currentUser,
                roleName,
    ...
    private void validateCreateUserInput(
        ...
        if (!tenDangNhap.matches("^[a-zA-Z0-9._-]+$")) {
            throw new AppException(HttpStatus.BAD_REQUEST, "Tên đăng nhập chỉ được chứa chữ, số, dấu chấm, gạch dưới hoặc gạch ngang");
        }
    ...
    private Long resolveBranchWhenBranchManagerCreatesUser(
        ...
        if (!ROLE_THU_NGAN.equals(newUserRole)) {
            throw new AppException(HttpStatus.FORBIDDEN, "Quản lý chi nhánh chỉ được tạo tài khoản THU_NGAN");
        }
    ...
    private Long resolveBranchWhenAdminCreatesUser(
            String newUserRole,
            Long requestedBranchId
    ) {
        ...
        if (requestedBranchId == null) {
            throw new AppException(HttpStatus.BAD_REQUEST, "Tài khoản không phải ADMIN phải có mã chi nhánh");
        }

        if (!nguoiDungRepository.existsActiveBranch(requestedBranchId)) {
            throw new AppException(HttpStatus.BAD_REQUEST, "Chi nhánh không tồn tại hoặc không hoạt động");
        }
