...
public class RecipeService {
  private static final Set<String> PRODUCT_STATUSES =
      Set.of("AVAILABLE", "OUT_OF_STOCK", "STOP_SELLING");
  ...
  public RecipeDetailResponse createRecipe(CreateRecipeRequest request) {
    ...
    Long maSanPham =
        recipeRepository.insertProduct(
            name, normalizeText(request.hinhAnh()), request.giaBanHienTai(), status);
  ...
  public RecipeDetailResponse updateRecipe(Long maSanPham, UpdateRecipeRequest request) {
    ...
    recipeRepository.updateProduct(
        maSanPham, name, normalizeText(request.hinhAnh()), request.giaBanHienTai(), status);
    ...
    for (RecipeIngredientRequest item : items) {
      ...
      if (!usedIngredients.add(maNguyenLieu)) {
        throw new AppException(
            HttpStatus.CONFLICT, "Một nguyên liệu không được lặp lại trong cùng công thức");
      }
      ...
      recipeRepository.insertFormulaItem(maSanPham, maNguyenLieu, item.soLuongCan());
    }
  }
  ...
  private String normalizeStatusRequired(String status, String defaultValue) {
    String value =
        status == null || status.isBlank() ? defaultValue : status.trim().toUpperCase(Locale.ROOT);
