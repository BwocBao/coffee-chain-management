...
public class StocktakeService {
    ...
    public StocktakeLookupResponse getLookups(SessionUser user) {
        ...
        response.setHandlingOptions(List.of(
                new StocktakeLookupResponse.OptionDto(null, NO_ACTION, "Không xử lý", "Chỉ ghi nhận chênh lệch"),
                new StocktakeLookupResponse.OptionDto(null, ADJUST_STOCK, "Điều chỉnh tồn", "Cập nhật lô và tồn kho theo số lượng thực tế"),
                new StocktakeLookupResponse.OptionDto(null, CREATE_WASTAGE, "Tạo hao hụt", "Ghi nhận phần thiếu hụt thành phiếu hao hụt")
        ));

        response.setStatuses(List.of(
                new StocktakeLookupResponse.OptionDto(null, DRAFT, "Phiếu nháp", "Phiếu đang nhập dữ liệu"),
                new StocktakeLookupResponse.OptionDto(null, COMPLETED, "Hoàn tất", "Phiếu đã hoàn tất kiểm kho"),
    ...
    public List<StocktakeResponse> searchStocktakes(
        ...
        List<StocktakeResponse> rows = stocktakeRepository.searchStocktakes(
                allowedWarehouseId,
                status,
                fromDate,
                toDate,
    ...
    public StocktakeResponse getById(SessionUser user, Long id) {
        ...
        StocktakeResponse response = stocktakeRepository.findById(id, forcedWarehouseId)
                .orElseThrow(() -> new AppException(HttpStatus.NOT_FOUND, "Không tìm thấy phiếu kiểm kho"));
    ...
    public StocktakeResponse createStocktake(SessionUser user, CreateStocktakeRequest request) {
        ...
        Long stocktakeId = stocktakeRepository.insertHeader(
                allowedWarehouseId,
                user.getMaNguoiDung(),
                ghiChu
        );
    ...
    @Transactional(rollbackFor = Exception.class)
    public StocktakeResponse updateStocktake(SessionUser user, Long id, UpdateStocktakeRequest request) {
        /*
        ...
        if (!DRAFT.equals(header.getTrangThai())) {
            throw new AppException(HttpStatus.BAD_REQUEST, "Chỉ được sửa phiếu kiểm kho ở trạng thái DRAFT");
        }
    ...
    public StocktakeResponse completeStocktake(SessionUser user, Long id) {
        ...
        if (!DRAFT.equals(header.getTrangThai())) {
            throw new AppException(HttpStatus.BAD_REQUEST, "Chỉ được hoàn tất phiếu kiểm kho ở trạng thái DRAFT");
        }
    ...
    public StocktakeResponse cancelStocktake(SessionUser user, Long id) {
        ...
        if (!DRAFT.equals(header.getTrangThai())) {
            throw new AppException(HttpStatus.BAD_REQUEST, "Chỉ được hủy phiếu kiểm kho ở trạng thái DRAFT");
        }
    ...
    private void processCompleteItem(
        ...
        if (maLoHang == null) {
            throw new AppException(
                    ...
                    "Dòng kiểm kho có xử lý tồn bắt buộc phải chọn lô hàng"
            );
        }
        ...
        StocktakeRepository.StockLock stock = stocktakeRepository.findStockForUpdate(
                maKho,
    ...
    private void applyStocktakeWastage(
        ...
        Long wastageId = stocktakeRepository.insertWastageFromStocktake(
                maKho,
                item.getMaNguyenLieu(),
                lot.getMaLoHang(),
                wastageQuantity,
    ...
    private void insertItems(Long stocktakeId, List<StocktakeItemRequest> items) {
        ...
        for (StocktakeItemRequest item : items) {
            ...
            if (!uniqueKeys.add(uniqueKey)) {
                throw new AppException(HttpStatus.CONFLICT, "Một lô/nguyên liệu không được kiểm trùng trong cùng phiếu");
            }
    ...
    private void validateItems(List<StocktakeItemRequest> items) {
        if (items == null || items.isEmpty()) {
            throw new AppException(HttpStatus.BAD_REQUEST, "Phiếu kiểm kho phải có ít nhất một dòng chi tiết");
        }
        ...
        for (StocktakeItemRequest item : items) {
            ...
            if (item.getSoLuongHeThong() == null || item.getSoLuongHeThong().compareTo(BigDecimal.ZERO) < 0) {
                throw new AppException(HttpStatus.BAD_REQUEST, "Số lượng hệ thống không được âm");
            }

            if (item.getSoLuongThucTe() == null || item.getSoLuongThucTe().compareTo(BigDecimal.ZERO) < 0) {
                throw new AppException(HttpStatus.BAD_REQUEST, "Số lượng thực tế không được âm");
            }
