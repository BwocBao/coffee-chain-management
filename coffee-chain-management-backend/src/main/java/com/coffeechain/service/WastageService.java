...
public class WastageService {
    ...
    public WastageLookupResponse getLookups(SessionUser user) {
        ...
        response.setWastageTypes(List.of(
                new WastageLookupResponse.OptionDto(null, EXPIRED, "Hết hạn", "Nguyên liệu quá hạn sử dụng"),
                new WastageLookupResponse.OptionDto(null, DAMAGED, "Hư hỏng", "Nguyên liệu bị hư hỏng"),
                new WastageLookupResponse.OptionDto(null, LOST, "Thất thoát", "Nguyên liệu bị thất thoát"),
                new WastageLookupResponse.OptionDto(null, SPILL, "Đổ vỡ", "Nguyên liệu bị đổ, rơi vãi")
        ));
    ...
    public WastageResponse getById(SessionUser user, Long id) {
        ...
        return wastageRepository.findById(id, forcedWarehouseId)
    ...
    public WastageResponse createWastage(SessionUser user, CreateWastageRequest request) {
        ...
        String loaiHaoHut = normalizeWastageTypeRequired(request == null ? null : request.getLoaiHaoHut());
        ...
        if ("USED_UP".equalsIgnoreCase(lot.getTrangThai())) {
            throw new AppException(HttpStatus.BAD_REQUEST, "Lô hàng đã dùng hết, không thể báo cáo hao hụt");
        }

        if (lot.getSoLuongConLai().compareTo(soLuongHaoHut) < 0) {
            throw new AppException(HttpStatus.BAD_REQUEST, "Số lượng hao hụt vượt quá số lượng còn lại của lô");
        }
        ...
        if (EXPIRED.equals(loaiHaoHut)
            ...
            throw new AppException(HttpStatus.BAD_REQUEST, "Lô hàng chưa hết hạn, không thể báo cáo loại hao hụt EXPIRED");
        }

        WastageRepository.StockLock stock = wastageRepository.findStockForUpdate(allowedWarehouseId, maNguyenLieu);

        if (stock == null) {
            throw new AppException(HttpStatus.BAD_REQUEST, "Không tìm thấy dòng tồn kho của nguyên liệu trong kho");
        }
        ...
        Long wastageId = wastageRepository.insertWastage(
                allowedWarehouseId,
                maNguyenLieu,
                maLoHang,
                soLuongHaoHut,
    ...
    private String normalizeWastageTypeRequired(String value) {
        ...
        if (!EXPIRED.equals(type) && !DAMAGED.equals(type) && !LOST.equals(type) && !SPILL.equals(type)) {
