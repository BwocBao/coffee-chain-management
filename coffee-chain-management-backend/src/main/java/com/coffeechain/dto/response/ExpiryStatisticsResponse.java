...
@Schema(description = "DTO response ExpiryStatisticsResponse. Swagger hien thi cac field backend tra ve cho frontend.")
public class ExpiryStatisticsResponse {
    @Schema(description = "Gia tri $field trong response tra ve frontend (tong so lo).", example = "1")
    private Integer tongSoLo;
    @Schema(description = "Gia tri $field trong response tra ve frontend (so lo dang hoat dong).", example = "1")
    private Integer soLoDangHoatDong;
    @Schema(description = "Gia tri $field trong response tra ve frontend (so lo sap het han).", example = "1")
    private Integer soLoSapHetHan;
    @Schema(description = "Gia tri $field trong response tra ve frontend (so lo da het han).", example = "1")
    private Integer soLoDaHetHan;
    @Schema(description = "Gia tri $field trong response tra ve frontend (so lo da dung het).", example = "1")
    private Integer soLoDaDungHet;
    @Schema(description = "Gia tri $field trong response tra ve frontend (so lo khong co han su dung).", example = "1")
