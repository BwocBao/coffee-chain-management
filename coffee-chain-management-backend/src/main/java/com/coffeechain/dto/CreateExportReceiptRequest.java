...
public class CreateExportReceiptRequest {
    @Schema(description = "Ma kho xuat hang", example = "1", requiredMode = Schema.RequiredMode.REQUIRED)
    private Long maKho;

    @Schema(description = "Loai xuat kho", example = "TRAINING", allowableValues = {"INTERNAL_USE", "RETURN_SUPPLIER", "TRAINING", "OTHER"}, requiredMode = Schema.RequiredMode.REQUIRED)
    private String loaiXuat;

    @Schema(description = "Bat che do chon lo thu cong. Neu false, backend tu xuat theo FEFO", example = "false")
    private Boolean chonLoThuCong = false;

    @Schema(description = "Ghi chu phieu xuat", example = "Xuat nguyen lieu cho dao tao")
    private String ghiChu;

    @Schema(description = "Danh sach nguyen lieu trong phieu xuat", requiredMode = Schema.RequiredMode.REQUIRED)
