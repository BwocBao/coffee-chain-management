...
public class AuthGuard {
    ...
    public SessionUser requireLogin(String authorizationHeader) {
        ...
        if (user == null) {
            throw new AppException(HttpStatus.UNAUTHORIZED, "Chưa đăng nhập hoặc phiên đăng nhập đã hết hạn");
        }
    ...
    public SessionUser requireAnyPermission(String authorizationHeader, String... permissions) {
        ...
        boolean allowed = Arrays.stream(permissions)
    ...
    public SessionUser requireRole(String authorizationHeader, String... roleNames) {
        ...
        boolean allowed = currentRole != null && Arrays.stream(roleNames)
                .filter(role -> role != null && !role.isBlank())
                .anyMatch(role -> role.equalsIgnoreCase(currentRole));

        if (!allowed) {
            throw new AppException(HttpStatus.FORBIDDEN, "Vai trò không được phép truy cập chức năng này");
        }
    ...
    public void requireSameBranchOrAdmin(SessionUser user, Long maChiNhanh) {
        ...
        if (maChiNhanh == null || user.getMaChiNhanh() == null || !user.getMaChiNhanh().equals(maChiNhanh)) {
    ...
    public String extractToken(String authorizationHeader) {
        ...
        if (!header.toLowerCase().startsWith("bearer ")) {
            throw new AppException(HttpStatus.UNAUTHORIZED, "Authorization header phải có dạng Bearer <token>");
        }
